...
es.obj {
    ...
    act = "Но кто меня ждёт? С кем я пришёл на пляж? Я пытаюсь вспомнить -- и не могу. Наверняка это из-за преждевременных сумерек -- они всегда способствуют забывчивости."
}
-- endregion
...
es.room {
    ...
    pic = "dream/lake2",
    disp = "Озеро",
    dsc = [[Я вымотался. Мышцы надсадно дёргаются, предупреждая о подступающих судорогах. Но берег не стал ближе. Напротив, он постоянно удаляется от меня, как будто его утаскивает в темноту окрепший с сумерками ветер. Я ложусь ненадолго на спину, чтобы отдохнуть и восстановить дыхание.]],
...
es.obj {
    nam = "moon1",
    dsc = "Огромная синяя {Луна} такая яркая, что слепит, как солнце в полдень, однако свет её вовсе не расходится волнами по небу. Вокруг меня собирается темнота.",
    act = "Я и не помню, чтобы по ночам была такая Луна!"
}
...
es.room {
    ...
    pic = "dream/lake1",
    disp = "Озеро",
    dsc = [[Я заплываю в пенящиеся волны, и что-то мигом утаскивает меня под воду, в кромешную темноту. Я пытаюсь выплыть, сражаясь с жуткой стихией, тянущей меня на дно. У меня ничего не получается.
    ^Я делаю последний рывок, трачу все оставшиеся силы, и всё же умудряюсь подняться над поверхностью.]],
    enter = function(s)
...
es.obj {
    ...
    act = function(s)
        if not s.done then
            s.done = true
            enable "#swim3"
            return "Я ложусь на спину и щурюсь от света синей Луны. Теперь я понимаю, что стоит избегать тех мест, где кружится пена. Видимо, там, глубоко под поверхностью, образовались омуты, которые вмиг утянут к смерти неопытного пловца. Днём они, наверное, боятся солнечного света."
        else
...
es.obj {
    nam = "left1",
    dsc = "забрать {чуть левее}, где чувствуются подводные потоки (но я, возможно, ещё смогу их побороть)",
    act = "Следов водоворотов я в этом направлении не вижу."
}

es.obj {
    nam = "right1",
    dsc = "или {повернуть направо}, выбрать самый длинный заплыв сквозь темноту, где нет никакого сопротивления, и течение, напротив, помогает мне держаться над водой.",
    act = "Кажется, вода в этом направлении тоже пенится, но пену мог занести поднявшийся ветер. Почему бы не рискнуть?"
}
-- endregion
...
es.room {
    ...
    pic = "dream/lake1",
    disp = "Озеро",
    dsc = [[Я уже сомневаюсь, что смогу выплыть. Силы иссякают. Чёрная вода не отпускает меня. Берег теперь кажется миражом, который отдаляется от меня с каждым взмахом рук.]],
...
es.obj {
    ...
    act = "Я приглядываюсь, и мне кажется, что я замечаю тусклый огонёк, который зовёт меня с берега, подсказывает правильный путь. А ещё я вижу пену, которая бурлит впереди."
}
...
es.room {
    ...
    alldsc = {
        ...
        "Вокруг меня на поднявшихся из-за ветра волнах играют отблески Луны.",
        "Кажется, берег так далеко, что скоро и вовсе исчезнет в темноте. Может, зря я сражаюсь с течениями? Может, они хотят помочь мне, вернуться меня на сушу?",
        "Я обязательно должен выплыть, я же -- отличный пловец. К тому же меня ждут. Меня точно ждут. Она наверняка уже волнуется.",
        "Такое ощущение, что я плаваю здесь несколько дней подряд.",
        "Становится совсем темно, как глубокой ночью, усиливается ветер, обдавая меня брызнами и пеной. Меня трясёт от холода."
    },
    dsc = function(s)
...
es.room {
    ...
    pic = "dream/lake2",
    disp = "Озеро",
    dsc = [[Я плыву вперёд, и спасительный свет становится ярче. Он придаёт мне сил.
...
es.obj {
    nam = "try_to_swim",
    dsc = "Я {сражаюсь} с подводным потоком.",
    act = function(s)
        es.music("horn")
        walkin("wait_wakeup")
    end
...
es.room {
    ...
    disp = "Озеро",
    dsc = [[Я не успеваю сделать и несколько взмахов руками, как меня утаскивает под воду, в бурлящую темноту. Я пытаюсь вырваться из ледяных объятий мрака, но мышцы сводит судорогой, и я сдаюсь стихии.]],
    next = "death1"
}
-- endregion
...
es.room {
    ...
    pic = "dream/lake.deep3",
    disp = "Озеро",
    dsc = [[Я не могу больше плыть, не могу сопротивляться настойчивому потоку, который тянет меня всё дальше от берега, увлекает за собой в темноту. Я пытаюсь лечь на воду, чтобы восстановить силы, но вместо этого медленно ухожу под воду.]],
    next = "death1"
}
-- endregion
...
es.room {
    ...
    noinv = true,
    pic = "common/report",
    dsc = [[Отчёт Елены Викторовны Ефимовой, главного врача НИОС "Кабирия"
    ^День 3872, 04:13 у.в.
    ^^Вереснев Олег Викторович, техник ГКМ "Грозный", 26 полных лет, умер во время промывания крови в медблоке станции. Точные причины смерти устанавливаются.]],
    enter = function(s)
        es.music("death")
    end,
    next_disp = "RELOAD",
...
es.room {
    ...
    dsc = [[С резким хрипом, как утопающий, я выныриваю из сна.
    ^Лёгкие раздирает от боли. В глаза бьёт пронизывающий свет. Я прикрываюсь ладонью и тут же падаю в слепяющую пустоту, врезаясь бедром в какую-то твёрдую поверхность.
    ^Руку пробивает яростным разрядом боли -- я представляю, как на предплечье лопается кожа, и из рваной раны вытягивают все жилы.
    ^Спустя пару секунд я наконец прихожу в себя и понимаю, что лежу на полу рядом с койкой. Из руки торчит толстая трубка катетера.]],
    enter = function(s)
        es.music("horn", 1, 0, 3000)
    end,
    next = function(s)
        es.music("onechance")
        walkin("intro1")
...
es.room {
    ...
    pic = "station/med1",
    disp = "Отсек",
    dsc = [[Я нахожусь в небольшом отсеке с привинченной к полу койкой у стены. С потолка за мной бесстрастно наблюдает сферический глазок камеры.]],
    preact = function(s)
        if not s.mus then
            s.mus = true
    ...
    onexit = function(s, t)
        if t.nam == "main" then
            if not all.catheter.done then
                p "Для начала нужно избавиться от катетера."
                return false
            else
                p "Дверь закрыта."
                return false
...
es.obj {
    ...
    act = function(s)
        if all.catheter.done then
            ...
            return "Надо сначала избавиться от катетера."
        end
    end,
    used = function(s, w)
        if not all.catheter.done then
...
es.obj {
    nam = "panel1",
    dsc = "Рядом с дверным проёмом есть небольшой, едва заметный {лючок}, который словно пытались спрятать от глаз.",
    act = function(s)
        if not all.catheter.done then
            return "Я ничего не могу сделать, пока не снял катетер."
        else
            return "Руками открыть лючок не получится. Тут нужен сервисный ключ, но, возможно, получится обойтись и без него."
        end
    end,
    used = function(s, w)
        if not all.catheter.done then
...
es.obj {
    nam = "smell1",
    dsc = "Воздух насквозь пропитан обжигающей {химией}, как будто здесь травили паразитов -- или пытались избавиться от других запахов.",
...
es.obj {
    ...
    cnd = "not s.done",
    dsc = "Я по-прежнему не могу отойти от кровати. {Трубку катетера} припаяли к нервам, вся рука немеет от боли.",
    act = function(s)
        if not all.patch.done then
            return "Я пытаюсь выдернуть трубку и не могу -- руку тут же обжигает кипятком боли."
        else
            s.done = true
            return "Трубка катетера теперь снимается без усилий. Руку, правда, ломает так, словно раздроблены кости."
        end
    end,
    used = function(s, w)
        if w.nam == "putty" then
...
es.obj {
    ...
    act = function(s)
        if not s.edge then
            ...
            s.done = true
            return "Я набираю побольше воздуха в грудь и со всей силы дёргаю пластырь. Нервные окончания рассекает разряд боли. На освобождённой коже тут же выступает кровь."
        end
    end,
    used = function(s, w)
        if w.nam == "putty" and not s.edge then
            s.edge = true
            return "Я осторожно подцепляю пластырь инструментом -- и как будто сдираю этой стальной лопаткой собственную кожу. Сделать это быстро, одним махом не получается. Больно так, как если бы я и правда пытался сам себя освежевать, но пластырь отклеился лишь наполовину."
...
es.obj {
    ...
    tak = [[Дотянуться до инструмента непросто, приходится натянуть трубку катетера, из-за чего рука отзывается такой болью, что темнеет в глазах.
    ^Но шпатель наконец-то у меня.]]
}
-- endregion
...
es.room {
    ...
    dsc = function(s)
        if not s.emergency then
            return [[Модуль большой, как и у Марутяна, только вместо одной широкой кровати -- две, стоящие у противоположных стен. Лампа и правда мерцает, а в воздухе стоит запах пыли -- сразу становится ясно, что здесь давно никто не живёт.]]
        else
    ...
    preact = function(s)
        if s.emergency and not s.mus2 then
            s.mus2 = true
            es.music("bass", 2, 0, 3000)
        elseif not s.emergency and not s.mus1 then
            s.mus1 = true
            es.music("whatif", 4, 0, 3000)
        end
    end,
    onenter = function(s)
        if not s.saved then
...
es.obj {
    ...
    dsc = function(s)
        ...
        elseif all.b1.emergency then
            return "{Выключатель света} на стене свет не выключает, и взбесившаяся лампа заливает глаза кровавыми туманом."
        else
...
es.obj {
    ...
    dsc = function(s)
        if all.b1.emergency then
            return "С {кровати} у левой стены содрали матрас, и по её голому каркасу пляшут алые отблески."
        else
            return "Обе кровати, разделённые падающей клином тенью, напоминают жёсткие больничные койки. У {левой кровати} зачем-то содрали матрас."
        end
    end,
    act = function(s)
...
es.obj {
    ...
    inv = function(s)
        if s.fixed and s.wear then
            s.wear = false
            return "Кисть потеет. Я снимаю перчатку."
        elseif s.fixed and not s.wear then
            s.wear = true
            return "Я натягиваю перчатку на руку. Лента пока держится."
        else
...
es.obj {
    ...
    dsc = function(s)
        if all.b1.emergency then
            return "Доживающая последние минуты лампа до {второй кровати} уже не дотягивается, и свет, точно дыхание, рассеивается в тени."
        else
...
es.obj {
    ...
    dsc = function(s)
        if all.b1.emergency then
            return "На кровати валяется {рулон с клейкой лентой}."
        else
            return "На кровати валяется {рулон с клейкой лентой} -- жёлтого цвета, с чёрной штриховкой."
        end
    end,
    act = function(s)
        s.taken = true
        take("tape")
        return "Я поднимаю ленту."
    end
}
...
es.obj {
    ...
    inv = "Жёлтая лента с разметкой. Такой лентой обычно заклеивают двери, которые нельзя открывать. Видимо, модуль хотели опечатать, но забыли или передумали.",
    used = function(s, w)
...
es.obj {
    ...
    used = function(s, w)
        ...
        elseif w.nam == "wire" or w.nam == "wirepin" or w.nam == "hairpin" then
...
es.obj {
    ...
    inv = "Для того, чтобы выдрать её, потребовалось не так уж и много усилий. Кстати, если схватить трубку за конец провода, то получится импровизированное оружие. Только вот сражаться не с кем."
}

es.obj {
    nam = "dirtybox",
    examed = false,
    dsc = "У стены неподалёку от двери стоит массивная пластиковая {коробка}.",
    act = function(s)
        s.examed = true
...
es.obj {
    ...
    act = function(s)
        if not all.b1.emergency then
            return "Розетка как розетка, нечего тут разглядывать."
        else
    ...
    used = function(s, w)
        if w.nam == "splinter" then
            return "Розетка сделана из толстого крепкого пластика и надёжно впаяна в стену. Содрать её корпус осколком стекла не получится."
        elseif w.nam == "wire" then
        ...
        elseif w.nam == "hairpin" and w.done then
            es.sound("electricity")
            return [[Я засовываю заколку в розетку, и вздрагиваю, когда меня мелкой дрожью пробивает разряд тока.
        ...
        elseif w.nam == "wirepin" then
            if not all.switch.wire and all.glove.wear then
                return "Я толкаю заколку в розетку -- никакого эффекта."
            elseif not all.switch.wire and not all.glove.wear then
                es.sound("electricity")
                return [[Я засовываю заколку в розетку, и вздрагиваю, когда меня мелкой дрожью пробивает разряд тока.
                ^-- Осторожнее! -- кричит Вера.
                ^И правда, не стоило быть настолько безрассудным. К тому же дверь по-прежнему закрыта.]]
            ...
            elseif all.switch.wire and all.glove.wear then
                ...
                purge("splinter")
                purge("tape") --?
                all.glove.wear = false
                es.music("ebomb", 1, 0, 4000)
...
es.obj {
    ...
    disp = es.tool "Шпилька",
    inv = function(s)
        if not s.done then
            return "Шпилька покрыта эмалью. Эмаль надо сначала соскоблить."
        else
            return "Шпилька для волос с соскобленной эмалью, отлично проводит ток."
        end
    end,
    used = function(s, w)
        if w.nam == "wire" then
...
es.obj {
    ...
    inv = function(s)
        if all.switch.wire then
            return "Провод поведён к останкам выключателя света и находится над напряжением. Остался последний шаг."
        else
            return "Провод от интеркома с примотанной к нему заколкой -- всё готово для саботажа электросети."
        end
    end
}
-- endregion
...
es.room {
    nam = "b1_latrin",
    pic = function(s)
        if not all.mirror.broken then
            return "station/toilet1"
        else
            return "station/toilet2"
        end
    end,
    disp = "Санузел",
    dsc = function(s)
        if all.b1.emergency then
            return "Здесь чуть тише, надрывный вой сирены становится приглушённым, а дверь отрезает от бешёной лампы, расплёскивающей повсюду кровавый свет."
        else
...
es.obj {
    ...
    used = function(s, w)
        ...
        elseif w.nam == "handset" then
            ...
            ^Зеркало вместе с трубкой разлетаются фонтаном осколков. Я едва успеваю отвернуться, что-то режет меня по щеке.]]
        end
    end
}
...
es.obj {
    ...
    inv = function(s)
        if all.switch.wire then
            return "Витой провод подключён одним концом к останкам выключателя света."
        else
...
es.obj {
    ...
    inv = function(s)
        ...
        elseif all.b1.madness then
            s.try = true
            return "Я смотрю на осколок зеркала в руке, сжимаю его так сильно, что он режет кожу и думаю -- может, и правда токсин ещё у меня в крови?"
        else
            return "Острый осколок зеркала, таким при желании можно перерезать горло."
        end
    end,
    used = function(s, w)
...
es.obj {
    ...
    act = "Здесь пытались уничтожить следы человеческого присутствия, но как-то нехотя и впопыхах.",
    used = function(s, w)
        if w.nam == "wool" and not w.powder then
            w.powder = true
...
es.room {
    ...
    disp = "Жилой модуль B2",
    dsc = [[Я набираю побольше воздуха в грудь и засовываю шпильку в розетку. В ту же секунду в грудь мне врезается огромный таран. Меня отбрасывает на спину, и я проваливаюсь в темноту.]],
    enter = function(s)
        es.music("ebomb")
    end,
    next_disp = "RELOAD",
...
es.room {
    ...
    pic = "station/corridor2",
    disp = "Коридор",
    dsc = [[В коридоре сирена захлебнулась, сменившись звоном в ушах. Аварийный режим отключился. Свет теперь горел ровно и ярко.
...
es.room {
    ...
    pic = "station/corridor2",
    disp = "Коридор",
    dsc = [[Мы остановились, чтобы отдышаться -- и решить, куда дальше идти. Коридор оглушительно пуст, как будто всё население станции эвакуировалось, пока орала сирена.]],
    onexit = function(s, t)
        if t.nam == "main" then
...
es.obj {
    ...
    dsc = "{Вера} стоит у переборки и касается её ладонью, прислушивается к пульсу.",
    branch = function(s)
    end,
    act = function(s)
        enable "#elevator"
...
es.obj {
    nam = "sunlight",
    dsc = "Узкий, обведённый жирной полосой спёкшейся грязи {иллюминатор} поймал луч звезды Сантори и усилил его, как линза.",
    act = "Странно, но до этого я ни разу не видел в иллюминаторе местное светило, оно пряталось от меня в складках темноты."
}
-- endregion
...
es.room {
    ...
    dsc = [[Свет на лифтовой площадке горит ярче, точно на сцене, из-за чего возникает гнетущее чувство, что все мы учавствуем в какой-то абсурдной постановке на орбите газового гиганта, в тысячах световых лет от Земли.]],
    onexit = function(s, w)
...
es.obj {
    ...
    dsc = function(s)
        if not all.andreev.wake then
            return "{Вера} нерешительно замерла перед распростёртым телом, словно тоже перестала верить в реальность происходящего."
        else
...
es.obj {
    nam = "elevator",
    dsc = "{Кнопка} вызова лифта не светится -- видимо, по протоколу безопасности все лифты на станции отключены, -- причём по табло над сомкнутыми дверями видно, что лифт стоит на нашем уровне.",
    act = function(s)
        if not all.andreev.wake then
...
es.obj {
    ...
    act = function(s)
        ...
        elseif not s.done then
            s.done = true
            return "Часть креплений сорвана, и лестница шатается, лязгая о металлическую переборку, стоит лишь слегка её тронуть. Вряд ли она выдержит даже вес Веры. Лучше здесь не подниматься."
        else
...
es.room {
    ...
    pic = "station/corridor2",
    disp = "Коридор",
    dsc = [[Я хожу по коридору, соединяющему жилые блоки, уже несколько дней, но сейчас он кажется невозможно длинным, словно мы идём по кругу через бесконечно одинаковые пролёты.]],
    onexit = function(s, t)
...
es.obj {
    ...
    act = function(s)
        ...
        elseif all.pane.found then
            ...
            all.pane.found = true
            return "Я подхожу ближе и замечаю, что панель воздуховода над иллюминатором почти слетела с петель."
        end
    end
}
...
es.obj {
    ...
    dsc = function(s)
        if not s.cracked then
            return "На {магнитном замке} быстро мигает красный светодиод."
        else
    ...
    used = function(s, w)
        if w.nam == "pane_piece" then
            ...
            purge("pane_piece")
            s.cracked = true
            return "Я сбиваю коробку замка. Оголяю электрические кишки. Достаточно быстрого осмотра, чтобы понять причину неполадки -- из-за тряски разошлись два проводка."
        end
    end
}
...
es.obj {
    ...
    act = function(s)
        ...
        elseif not all.glove.wear then
            ...
            return "Всего-то и нужно было скрутить два проводка -- огонёк светодиода тут же сменяется на зелёный. Дверь открыта."
        end
    end,
    used = function(s, w)
        if w.nam == "glove" then
...
es.room {
    ...
    enter = function(s)
        es.music("descend", 2, 0, 3000)
    end,
    next = "interlude4_next"
}
-- endregion

-- region interlude4_next
es.room {
    nam = "interlude4_next",
    pic = "station/staircase",
    disp = "Лестничная клетка",
    dsc = [[Я поднялся на верхний уровень и оказался в узком, как гроб, отсеке -- перед закрытой дверью.]],
    next = function(s)
        es.music("juxtaposition", 1, 0, 4000)
        es.walkdlg {
            dlg = "vera",
            branch = "closed",
            pic = "station/staircase",
...
es.obj {
    ...
    act = "Где-то здесь должна быть кнопка. Если бы только горел свет..."
}
...
-- region before_corridor3
es.room {
    nam = "before_corridor3",
    noinv = true,
    pause = 60,
    enter = function(s)
...
es.room {
    ...
    disp = "Коридор",
    dsc = [[Всё вокруг дрожит, как в припадке.
    ^Гравитационные катушки едва тянут. Сначала мне кажется, что я вот-вот оторвусь от пола и поплыву в невесомости, а спустя секунду ноги становятся свинцовыми, и я чуть не валюсь на колени.]],
    onexit = function(s, t)
...
es.obj {
    nam = "figures",
    dsc = "Я замечаю в глубине коридора {двух людей}, две вытянутые, как в сумерки, тени -- один плавит какую-то дверь плазменным резаком, отворачиваясь от снопа искр, а второй, будто в приступе бешенства, барабанит по ней кулаком.",
    act = function(s)
...
es.room {
    ...
    ^Стена передо мной лопается, точно бумажный лист, и наружу, как органы освежёванного животного, вываливаются мотки проводки и рвущиеся со стоном трубы. Что-то взрывается фонтаном огненных брызг, полоска огня быстро взбирается по стене. По воздуху разливается запах химической гари.
...
es.room {
    ...
    ^Прихожу в себя на полу -- я лежу, распростёршись, как труп. Рядом валяются бессвязные обломки, куски обшивки, смятые трубы, погнутые панели. Затылок горит от боли. Как будто голова пробита насквозь металлическим штырём.
...
es.room {
    ...
    ^Он лежит рядом, на боку, поджав под себя ноги, словно спит, но глаза у него открыты, и он смотрит на меня тяжёлым невидящим взглядом.
    ^Я говорю, но слова как-то путаются, цепляются друг за друга, утрачивая даже тень смысла. Я говорю долго -- так, по крайней мере, кажется. Я всё пытаюсь ему что-то объяснить, втолковываю про операционные модули, нуболидов, прочность камеры содержания. 
    ^Он не отвечает.
...
es.room {
    ...
    pic = "station/impact5",
    disp = "Коридор",
    dsc = [[С потолка, как град, сыплятся осколки. Я невольно прикрываю голову. Раздаётся мелодичный голос системы оповещения:
    ^-- В случае отказа двигательных установок, схода с орбиты или другой эквивалентной катастрофы... -- Неживой голос скатывается в звон, сбивается и начинает заново, как сорванная с незаконченной песни пластинка: -- В случае отказа двигательных установок, схода с орбиты или другой эквивалентой катастрофы...
    ^Мимо меня тащат за руки окровавленное тело.]],
    next = "interlude10"
}
-- endregion
...
es.room {
    ...
    pic = "station/impact6",
    disp = "Коридор",
    dsc = [[Кто-то подхватывает меня под руки. Я не сопротивляюсь. Мне хочется сказать, что я ещё жив, но я не могу разлепить рот. Наверное, сейчас меня потащат в красное марево, как других мертвецов.
    ^Когда умираешь, всё вокруг застилает кровавый туман.
    ^Меня усаживают у стены. Надо мной склоняется мужчина с впалыми, как у больного, щёками.
    ^Это Мицюкин.]],
    next = function(s)
        es.music("tragedy", 2, 4000, 6000)
        es.walkdlg {
            dlg = "mitsukin",
            branch = "help",
...
es.room {
    ...
    pic = "station/corridor_red2",
    disp = "Коридор",
    dsc = [[Кто-то стонет. Подыхающие люминофоры расплескивают повсюду свет, напоминающий зарево пожара. Где-то и правда горит.]],
    enter = function(s)
        if not s.mus then
            s.mus = true
            es.music("doom", 4, 0, 3000)
        end
    end,
    onexit = function(s, t)
...
es.obj {
    ...
    dsc = "{Тело Алексина} прикрыли красным пластиковым мешком, из которого выпросталась бледная и тонкая рука.",
    act = function(s)
        if not s.done then
            s.done = true
            walkin("alexin_scene")
            ...
            return "Я не могу на это смотреть."
        end
    end
}
...
es.obj {
    ...
    ^Я обхватываю арматуру обеими руками, тяну. За переборкой что-то надрывно трещит, ноги скользят по липкому полу.
    ^Нет, так её не вытащить.]],
    ...
    used = function(s, w)
        if w.nam == "cutter" then
            ...
            s.taken = true
            take("stick")
            return "Я срезаю плазменной горелкой кусок арматуры."
        end
    end
}
...
es.obj {
    ...
    dsc = function(s)
        if not s.done then
            return "{Лысанов} делает кому-то искусственное дыхание, надрывно, как из последних сил, давит на грудную клетку. Пот стекает у него по лицу."
        else
...
es.obj {
    ...
    act = [[Я какое-то время смотрю на это гипнотизирующее мерцание, как если бы сходящая с ума электрика и правда передавала мне последнее послание.]]
}
...
es.obj {
    ...
    dsc = function(s)
        if s.fire and not s.firefixed then
            return "^^{Мицюкин} и Лысанов стоят у перекошенной двери в отсек и шумно переругиваются."
        else
...
es.room {
    ...
    ^Я опускаюсь на колени, касаюсь полиэтилена и -- замираю. Нет, я не могу больше смотреть в его мёртвые глаза.]],
    next = "redcor1"
}
-- endregion
...
es.room {
    ...
    pic = "station/deck",
    disp = "Рубка",
    dsc = [[Рубка пострадала куда меньше коридора -- видно, стены здесь крепче, чем в других отсеках.]],
    enter = function(s)
        if not s.mus then
            s.mus = true
...
es.obj {
    nam = "destruction",
    dsc = "Пара ламп полопалась. Судя по подпалинам на потолке, небольшие {возгорания} были и в рубке.",
    act = "К счастью, огонь не перекинулся на технику."
}

es.obj {
    nam = "terminals",
    dsc = "Однако даже {терминалы}, выстроенные перед тремя креслами, по-прежнему работают.",
...
es.obj {
    nam = "body",
    branch = "body",
    dsc = "В одном из кресел сидит {мужчина}. Он никак на нас не реагирует и смотрит в мерцающий монитор.",
    act = function(s)
        all.deck.done = true
...
es.room {
    ...
    disp = "Коридор",
    dsc = [[От криков, стонов, треска проводки идёт кругом голова. Ноги подгибаются от усталости, под череп заливают расплавленный свинец.]],
    onexit = function(s, t)
        if t.nam == "redcor3" and 
            not (all.deck.done and all.minaeva.done and all.kofman.done) then
            p "Я делаю несколько шагов и останавливаюсь. Звезда Сантори зло светит в глаза. Надо сначала помочь пострадавшим. Я не могу просто взять и уйти."
            return false
        end
    end,
    obj = {
...
es.obj {
    ...
    dsc = function(s)
        if not s.done then
            ...
            return "{Дыра} в потолоке такая большая и чёрная, будто корпус станции пробило насквозь."
        end
    end,
    act = function(s)
        if not s.done then
...
es.obj {
    ...
    cnd = "not s.done",
    dsc = "Рядом прямо на полу полыхает {пламя} -- видимо, горит разлившийся конденсат.",
...
es.room {
    ...
    dsc = [[Разрушений у лестничной клетки меньше, но с потолка, сквозь длинные, точно прорезанные автогеном щели, всё равно свисает проводка. Пара ламп чудом горит, подкрашивая багрянцем стены.]],
...
es.room {
    ...
    pic = "station/staircase",
    disp = "Лестничная клетка",
    dsc = [[Света на лестничной клетке по-прежнему нет, и разглядеть тут что-либо довольно сложно.]],
    enter = function(s)
...
es.obj {
    nam = "lowdebris",
    dsc = "{Разрушений} в этой части коридора меньше. Но воздух заполнен едкой гарью, и большинство ламп полопалось.",
    act = "Кажется, стоит сделать ещё несколько шагов, и этот кошмар закончится, станция восстановится, загрузится из резервной копии, а в коридоре появятся беззаботные люди и поприветствуют меня вежливыми улыбками."
}
...
es.obj {
    ...
    act = "Местная звезда будто хочет испепелить станцию. Нам здесь не рады."
}
...
es.obj {
    ...
    dsc = function(s)
        if s.locked and not all.mitsukin2.nubolids then
            return "Огромный панорамный иллюминатор закрыт плотной металлической шторой, не пропускающей свет -- теперь можно представить на мгновение, что ничего не случилось, что вяло течёт обычный скучный день на орбитальной станции, а я просто проснулся после кошмарного сна."
...
es.obj {
    ...
    nubolids = false,
    cnd = "{lab_main}.done",
    dsc = "{Мицюкин} стоит у выхода в коридор и о чём-то разговаривает с Лысановым.",
    act = function(s)
...
es.room {
    ...
    disp = "Холл",
    dsc = function(s)
        if all.mitsukin2.nubolids then
            return [[От поверхности тянутся десятки протуберанцев -- планета, точно народившаяся звезда, испускает в темноту смертоносные лучи. Это пугает до дрожи и в то же время кажется необъяснимо красивым, будто бы посреди творящегося хаоса и смерти рождается новая жизнь.]]
        else
            all.santory5.locked = true
            return [[От поверхности тянутся десятки протуберанцев -- планета, точно народившаяся звезда, испускает в темноту смертоносные лучи. Это пугает до дрожи и в то же время кажется необъяснимо красивым, будто бы посреди творящегося хаоса и смерти рождается новая жизнь.
            ^Я смотрю в иллюминатор долго, как в трансе, пока рука сама не находит на стене переключатель. Опустившаяся металлическая штора скрывает бушующую планету.]]
        end
    end,
    next = "lab_hall"
}
...
es.obj {
    ...
    act = "Видимо, мы так и не закончим диагностику модулей."
}
...
es.terminal {
    ...
    commands_help = {
        ...
        read = "чтение файла из личного профиля",
...
es.room {
    ...
    disp = "Лаборатория",
    dsc = [[Нуболиды чувствуют моё присутствие. Они сплетаются в огромный клубок, похожий на кровавое щупальце и бьют по стеклу, размахиваются, снова бьют, с нарастающим темпом, с какой-то бешеной яростью, с ненавистью ломая разделяющий нас заслон.
    ^Через несколько секунд щупальце замирает, расплетается мириадой красных нитей и плавно, как под воздействием невидимых волн, складывается во что-то похожее на человеское лицо.
