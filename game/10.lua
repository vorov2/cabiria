...
es.obj {
    ...
    act = "Кажется, вода в этом направлении тоже немного пенится, но, возможно, пену занёс поднявшийся ветер. Почему бы не рискнуть?"
}
-- endregion
...
es.obj {
    ...
    act = "Я приглядываюсь, и мне кажется, что я замечаю тусклый огонёк, который зовёт меня с берега, подсказывает мне правильный путь. А ещё я вижу пену, которая бурлит прямо передо мной."
}
...
es.obj {
    nam = "try_to_swim",
    dsc = "Я всё равно {пытаюсь выплыть}.",
    act = function(s)
        es.music("horn")
        walkin("wakeup1")
    end
...
es.room {
    ...
    disp = "Озеро",
    dsc = [[Я не успеваю сделать и несколько взмахов руками, как меня утаскивает под воду, в бурлящую темноту. Я пытаюсь вырваться из ледяных объятий мрака, но мышцы быстро сводит судорогой, и я сдаюсь стихии.]],
    next = "death1"
}
-- endregion
...
es.obj {
    ...
    dsc = function(s)
        if all.b1.emergency then
            ...
            return "На кровати лежит {рулон с клейкой лентой} -- жёлтого цвета, с чёрной штриховкой."
        end
    end,
    act = function(s)
        s.taken = true
...
es.obj {
    ...
    used = function(s, w)
        ...
        elseif w.nam == "handset" then
            ...
            ^Зеркало вместе с трубкой разлетаются фонтаном осколков. Я едва успел отвернуться, и всё равно что-то резануло меня по щеке.]]
        end
    end
}
...
es.room {
    ...
    pic = "station/corridor2",
    disp = "Коридор",
    dsc = [[Мы остановились, чтобы отдышаться -- и решить, куда дальше идти. Коридор оглушительно пуст, как будто всё население станции эвакуировалось, пока орала сирена, бросив нас здесь одних.]],
    onexit = function(s, t)
        if t.nam == "main" then
...
es.obj {
    ...
    dsc = "{Вера} стоит у переборки и касаеется её ладонью, прислушивается к пульсу.",
    branch = function(s)
    end,
    act = function(s)
        enable "#elevator"
...
es.obj {
    nam = "sunlight",
    dsc = "Узкий, обведённый жирной полосой спёкшейся грязи {иллюминатор}, поймал луч звезды Сантори, и тот настойчиво бьёт в глаза.",
    act = "Странно, но до этого я ни разу не видел в иллюминаторе местное светило, оно пряталось от меня в складках темноты, утаскивая за собой остальные звёзды."
}
-- endregion
...
es.room {
    ...
    pic = "station/corridor2",
    disp = "Коридор",
    dsc = [[Я хожу по коридору, соединяющему жилые блоки, уже несколько дней, но сейчас он кажется невыносимо длинным, точно мы идём по кругу мимо бесконечно одинаковых пролётов.]],
    onexit = function(s, t)
...
es.room {
    ...
    ^Гравитационные катушки едва тянут. Сначала мне кажется, что я вот-вот оторвусь от пола и поплыву в невесмости, а спустя секунду ноги становятся свинцовыми, и я чуть не падаю на колени.]],
    onexit = function(s, t)
...
es.obj {
    nam = "destruction",
    dsc = "Пару ламп полопались. Судя по подпалинам на потолке, небольшие {возгорания} здесь тоже были.",
