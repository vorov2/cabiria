...
es.room {
    ...
    dsc = [[Несмотря на несколько чашек местного кофе, не уступающего по горечи рвотному, я чувствую себя как человек, которому не дают спать в виде изощрённой пытки.
...
es.room {
    ...
    pic = "station/tech",
    disp = "Технический отсек лаборатории",
    dsc = [[Отсек с оборудованием в лаборатории чем-то напоминает техничку на нашем корабле. Пространства здесь, конечно, гораздо больше, и потолки высокие, почти, как в земных постройках.]],
    onexit = function(s)
...
es.obj {
    ...
    cnd = "not s.done",
    dsc = "Я вдруг понимаю, что совершенно {забыл} всё, о чем он мне говорил по поводу сегодняшней работы.",
...
es.obj {
    ...
    problem = false,
    unlocked = false,
    dsc = "Мой {терминал}, поменьше, стоит в уголке на небольшом столике.",
    act = function(s)
        s.unlocked = false
...
es.obj {
    ...
    opened = false,
    dsc = function(s)
        local base = "Неподалёку от него можно обнаружить металлический {шкаф}, в котором обычно хранятся инструменты и запасное оборудование."
        if s.opened then
...
es.terminal {
    ...
    mod = function(s, args)
        ...
        elseif not tonumber(mod) then
            return false,
                "Некорректный формат модуля операционного модуля: "..mod
        else
            mod = tonumber(mod)
            local mk = _("module"..tostring(mod - 3))
            if mod < 4 or mod > 8 or not mk.active then
                return false, "Модуль не введён в сервисный режим для проверки."
            end
            return true, mk
        end
    end,
    commands = {
        ...
        elock = function(s, args, load)
            ...
            elseif key == "toolbox" then
                ...
                elseif not t.locked and t.opened then
                    return "Ошибка! Невозможно заблокировать электронный замок!"
                ...
                return "Неизвестный идентификатор оборудования: "..key
            end
        end,
        readlog = function(s, args, load)
            ...
            seed = tonumber(seed)
            if not seed then
                return "Некорректный формат номер модуля: " .. seed
            end
            if seed < 1 or seed > 9 then
                return "Неизвестный номер модуля: " .. seed
            end
            if not load then
                return "$load"
            end
        ...
        diag = function(s, args, loaded)
            ...
            elseif res.problem == "reset" or res.problem == "hard" then
                ...
                return string.format(
                    "Модуль %s: не отвечает на диагностические команды.", 
                    res:number())
            end
        end,
        read = function(s, args)
            local fl = s:arg(args)
            if not fl then
                return {
                    "Не задано имя файла.",
            ...
            elseif fl == "message-3471-11-23.txt" then
                return {
                    ...
                    "для восьмого модуля опять не привели. Официальный запрос, конечно же,",
            ...
            elseif fl == "message-3489-14-05.txt" then
                return {
                    "",
                    "Сергей, повторяться я не буду. Мы и так потеряли один модуль,",
                    "больше я рисковать так не намерен. Да, да, меня бы давно уже уволили,",
                    "если бы была прямая связь. Давай просто заниматься своей работой, а не",
                    "переводить стрелки. Согласен, ты не виноват в том, что десятый вышел",
                    "из строя. Доволен? А теперь давай работать.",
                    "",
                    "С уважением,",
                    "Арто Марутян"
                }
            elseif fl == "memo.txt" then
                return {
                    ...
                    "причину мы так и не установили, но сейчас это и не важно. Модуль должен",
                    "выдержать. Главное: если соединить его с восьмым, происходит",
                    "горячая перезагрузка, и восьмой снова начинает работать. Надеюсь мы",
                    "протянем таким образом до следующих поставок."
                }
            else
                return string.format("Файл \"%s\" не найден.", fl)
            end
        end,
        files = function(s, args)
            return {
                "Список файлов в личном профиле:",
                "dump-01312.dat",
                "dump-02476.dat",
                "message-3471-11-23.txt",
                "message-3487-16-24.txt",
                "message-3489-14-05.txt",
                "memo.txt"
            }
        end,
        modres = function(s, args, loaded)
            ...
            if res.problem == "hard" then
                ...
                if res:number() == 8 and ep1 and
                    ...
                    return {
                        string.format(
                            "Модуль %s: невозможно произвести перезагрузку.",
                            res:number())
                    }
                end
            else
...
es.obj {
    ...
    act = "У модулей есть только порядковые номера -- нет никакого другого способа понять, за что они отвечают, кроме как, изучить, чем соответствует тот или иной номер.",
    used = function(s, w)
        if w.nam == "key" then
    ...
    used = function(s, w)
        ...
        elseif w.nam == "cable" then
            ...
            elseif e1 == s.nam then
                ...
                purge("cable")
                return string.format(
                    "Я подключил кабель к %s и %s модулям. Ничего не произошло. Возможно, стоит покопаться в терминале, или подключить клеммы иначе.",
...
es.room {
    ...
    pic = "station/hall",
    disp = "Холл",
    dsc = [[Холл почему-то напоминает мне зал ожидания на каком-нибудь аэровокзале -- вскоре после того, как улетел очередной межконтинентальный лайнер, и привычный гомон сменяли мягкая сонная тишина.]],
    onexit = function(s, t)
        if t.nam == "main" then
...
es.obj {
    nam = "porthole",
    dsc = "Широкий {иллюминатор} напоминает экран, по которому круглые сутки показывают светящуюся в темноте, точно жемчужина, планету.",
    act = "Огромный вихрь теперь захватывает половину планеты. Интересно, как долго на Сантори может продолжаться ураганом?"
}
...
es.obj {
    nam = "aquarium",
    dsc = "{Нуболиды} в аквариуме сегодня взволнованы -- они носятся кругами, закручиваясь штопором, как в водовороте, словно хотят вырваться наружу.",
    act = function(s)
        walk("outro1")
        return true
    end
...
es.obj {
    nam = "labcomp",
    dsc = "{Терминал} рядом с аквариумом работает, но изображение на экране дёргается, как во время системной ошибки.",
    act = function(s)
        walk("lab.terminal")
        return true
    end
...
es.room {
    ...
    disp = "Лаборатория",
    dsc = [[Я подхожу ближе и касаюсь стекла рукой. Наболиды тут же замирают. Они чуствуют меня. Я даже не могу избавиться от мысли, что они каким-то образом видят, что я стою у контейнера.
    ^Словно в подтверждение моих слов алые полосы оживают и тянутся к моей руке, они касаются сквозь стекло моих пальцев. Меня пронизывают слабые разряды электричества.
    ^Я отдёргиваю руку.]],
    enter = function(s)
        es.music("horn")
    end,
