...
es.room {
    ...
    pic = "station/debris1",
    disp = "Коридор",
    dsc = [[Мне кажется, что каждый шаг может стать последним -- пол раскроит огромная трещина, меня затянет в кипящие огнём недра станции, -- и я придерживаюсь за трещащий леер в стене, как за спасательную верёвку.]],
    enter = function(s)
...
es.obj {
    nam = "screams",
    dsc = "В спину мне бьют чьи-то надрывные {крики} -- словно кто-то пытается кричать, когда воздух в лёгких почти вышел.",
...
es.obj {
    ...
    act = "Сантори-5 с рвущимися в темноту протуберанцами стала теперь куда ближе, чем раньше, и затягивает нас в своей некончающийся ураган."
}
...
es.room {
    ...
    dsc = [[В лаборатории стоит треск от работающий терминалов, перемигиваются индикаторы, по лупатым экраном ползут строчки трассировок. Как будто ничего не произошло.]],
    onenter = function(s)
...
es.obj {
    ...
    dsc = function(s)
        if not s.done then
            return "{Симонова} -- осунувшаяся, со спутавшимися волосами -- стоит у своего терминала и что-то быстро выщёлкивает на клавиатуре, не глядя на экран."
        else
            return "{Симонова} c"
        end
    end,
...
es.obj {
    nam = "simcomp2",
    dsc = "На экране её {вычислительного аппарата} быстро сменяются какие-то цифры.",
    act = "Вычислительный аппарат заблокирован, доступа без ключ-карты не вообще.",
    used = function(s, w)
...
es.obj {
    ...
    act = function(s)
        if all.lab.crisis then
            return "Кажется, они материализуются в камере из пустоты. Скоро она не выдержит."
        elseif not all.labcomp.tenebris then
...
es.obj {
    ...
    act = function(s)
        ...
        elseif not s.fixed then
            s.examed = true
            return "Кажется, вычислительный аппарат завис и не реагирует на нажатия клавиш. По экрану бегут бессвязные строчки каких-то логов и сообщения об ошибках."
        else
            s.unlocked = false
            walkin("lab.terminal")
            return true
    ...
    used = function(s, w)
        ...
        elseif w.nam == "servicekey" and all.simonova.talk then
            s.fixed = true
            return "Я вставляю сервисный ключ в разъём и поворачиваю его по часовой стрелке. Изображение на экране мигает, и начинается загрузка системы."
...
es.obj {
    ...
    dsc = function(s)
        if not s.taken then
            return "На стене у входа висит в прозрачном боксе {сервисный ключ}."
        else
            return "На стене у входа висит прозрачный {бокс} для сервисного ключа."
        end
    end,
    act = function(s)
        if not s.taken then
...
es.room {
    ...
    pic = "station/lab",
    disp = "Лабораторный отсек",
    dsc = [[Я ничего не успеваю сделать. Симонова хватает со стола тяжёлую клавиатуру терминала и с размаху бьёт Веру по голове. Та охает, отшатывается и падает на пол. Я подбегаю к ней, падаю на колени. Клавиатура падает на металлический пол, разлетаются повсюду колпачки от клавиш.
    ^-- Вера! Вера! -- кричу я.
    ^Она не отвечает.]],
    enter = function(s)
        es.music("death")
    end,
    next_disp = "Загрузить последнюю точку сохранения",
    next = function(s)
...
es.room {
    ...
    disp = "Лабораторный отсек",
    dsc = [[Потолок над ними угрожающе колышется, света почти нет. Лишь иногда сквозь плотную массу нуболидов пробивается красный просверк и отбрасывает на стены длинную тень. Мы в западне.]],
...
es.obj {
    nam = "vera2",
    dsc = "{Вера} прижимается к стене у выхода, с ужасом глядя на нуболидов. Видно, что она боится даже пошевелиться.",
    act = function(s)
...
es.room {
    ...
    pic = "station/hall",
    disp = "Холл",
    dsc = [[Такое чувство, что всё внутри разорвано на части, что я сам доживаю последние мгновения, и в любую секунду кровь польётся из глаз.]],
    onexit = function(s, t)
...
es.obj {
    ...
    dsc = function(s)
        if s.done then
            ...
            return "{Вера} стоит рядом с ней на коленях, размывая по щекам слёзы."
        end
    end,
    act = function(s)
        if not s.done then
...
es.room {
    ...
    dsc = function(s)
        if not all.lever.done then
            return [[Мы проходим несколько метров, и перед нами вырастает металлическая стена, преграждающая нам проход.]]
        else
...
es.obj {
    nam = "vera4",
    dsc = "{Вера} стоит рядом со мной, но мне всё равно кажется, что она рассеивается в подступающем к нам со спины мраке.",
    act = function(s)
        if not all.lever.done then
...
es.obj {
    ...
    act = "Надо быть осторожнее. Хоть бы ноги тут не переломать."
}
...
es.obj {
    ...
    act = function(s)
        if not s.unbar then
            ...
            ^Верхняя часть лестницы, покорёженный обрубок, сорвана со стены и застяла внутри спуска, точно кость в горле. Даже спрыгнуть вниз не получится.]]
        else
...
es.terminal {
    ...
    commands = {
        control = function(s, args)
            ...
            if not args[2] then
                return "Ошибка! Не указан идентификатор команды."
            end
            return string.format("Ошибка! Некорректный идентификатор команды: \"%s\".", args[2])
        end,
...
es.obj {
    ...
    dsc = "В пролегающей у основания стены тени прячётся какая-то невзрачная {коробка}.",
...
es.obj {
    ...
    dsc = "Мне кажется, что я замечаю в плотной завесе тьмы впереди какое-то {движение}.",
    act = function(s)
        if not s.done then
    ...
    used = function(s, w)
        if w.nam == "flash" then
            s.done = true
            return [[Я делаю несколько шагов вперёд, ощупывая лучом фонаря потолок и стены.
            ^Ничего.]]
        end
    end
}
...
es.room {
    nam = "death6",
    pic = "station/redtail",
    dsc = [[Я лезу первым, леер, как лезвия, врезается в кисти рук, но подниматься получается довольно быстро. Вера лезет вслед за мной.
...
es.room {
    ...
    dsc = [[Мы один за другим спускаемся из шлюза в коридор корабля. Здесь так ярко, что глаза, привыкшее к сумраку, начинают слезиться.]],
    obj = { "minaeva1", "majorov1", "vera19" }
}

es.obj {
    nam = "minaeva1",
    dsc = "На полу лежит {Минаева}. Её форма расстёгнута, а груди присосался чёрный диск реаниматора, который вращается по кругу, громко отщёлкивая секунды. Минаева вздрагиает, но не приходит в сознание.",
    act = "Что он делает? У нас же есть медицинская капсула!"
}
...
es.obj {
    ...
    act = "Можно потом попробовать сделать ей искусственное дыхание. Она не должна умереть!"
}
-- endregion
