...
es.room {
    ...
    pic = "station/debris1",
    disp = "Коридор",
    dsc = [[Мне кажется, что каждый шаг может стать последним -- пол раскроит жуткая трещина, меня затянет в кипящие огнём недра станции, -- и я придерживаюсь за трещащий леер в стене, как за спасательную верёвку.]],
    enter = function(s)
...
es.room {
    ...
    dsc = [[В лаборатории стоит треск от работающий терминалов, перемигиваются индикаторы, по лупатым экранам ползут строчки трассировок. Как будто ничего не произошло.]],
    onenter = function(s)
...
es.room {
    ...
    pic = "station/hall",
    disp = "Холл",
    dsc = [[Такое чувство, что всё внутри разорвано на части, что я сам доживаю последние мгновения, что в любую секунду кровь брызнет из глаз.]],
    enter = function(s)
        if not s.mus then
            s.mus = true
...
es.obj {
    nam = "minaeva1",
    dsc = "На полу лежит {Минаева}. Её форма расстёгнута, а к груди присосался чёрный диск реаниматора, который вращается по кругу, громко отщёлкивая секунды. Минаева вздрагиает, но не приходит в сознание.",
