...
es.room {
    ...
    Мерцель Елена Павловна, дейн-пилот, 46 лет^
...
es.obj {
    ...
    act = "Лист капитан в шутку повесил на переборку в каюте как напоминание, что я здесь -- единственный, кто ещё не переживал многодневный переход через голую пустоту вселенной. Распорядок дня. Как в школе."
}
...
es.obj {
    ...
    act = function(s)
        ...
        elseif s.done then
            ...
            s.done = true
            return "Я опускаю на рычаг, и из отверстия в потолке летят хлопья едкого, отдающего хлором порошка. Я несколько секунд втираю его в кожу. Всё, теперь от меня пахнет так, словно я много часов кряду плавал в общественном бассейне."
        end
    end
}
