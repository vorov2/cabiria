...
es.room {
    ...
    dsc = function(s)
        if all.lever2.done then
            ...
            return [[Я долго парю под потолком в каюте, собираясь с мыслями. Кровь пульсирует в висках. Прямые линии стен и потолка мреют перед глазами, превращаются в пологие волны. Я и сам плыву в эту обморочную муть, тону пропитанном едкой химией воздухе.]]
        end
    end,
    preact = function(s)
...
es.room {
    ...
    ^Время в полёте воспринимается иначе -- я словно десятки лет провёл на борту "Грозного", который хоть и считается самым передовым ГМК всесоюзного флота, запросто может свести с ума теснотой и многочисленными неудобствами.]],
    next = "cabin"
}
-- endregion
...
es.obj {
    ...
    act = function(s)
        ...
        elseif s.done then
            ...
            s.done = true
            return "Я опускаю на рычаг, и из отверстия в потолке сыплется хлопьями, как искусственный снег, едкий, отдающий хлором порошок. Я несколько секунд втираю его в кожу. Всё, теперь от меня пахнет так, словно я несколько часов кряду плавал в общественном бассейне."
        end
    end
}
...
es.obj {
    ...
    cnd = "not s.done",
    dsc = "Я расправляю плечи, потираю затёкшую шею -- суставы похрустывают так, словно я провисел в коконе для сна доброе десятилетие, а не несколько жалких часов. К тому же тяжёлое ощущение после сна так и не проходит -- {воздух} кажется вязким, как при отказе системе рециркуляции, и отдаёт металлическим привкусом, точно венозной кровью.",
    act = function(s)
        s.done = true
...
es.obj {
    ...
    dsc = function(s)
        if not s.taken then
            return "Также на стене у двери висит металлический кронштейн, где висит трёхгранный {ключ} -- главный сервисный инструмент на время вахты."
        else
...
es.obj {
    ...
    act = "Я касаюсь стекла ладонью -- пальцы пронизывает холод, как от искусственного льда. Стекло на секунду проясняется, и я вижу женщину в белоё больничной робе. У неё молодое, красивое и мёртвое лицо. Глаза закрыты, изо рта торчит толстая резиновая трубка, похожая на щупальце паразита, высасывающего жизнь."
}
...
es.obj {
    ...
    act = function(s)
        ...
        elseif c.num ~= ch.num then
            ...
            ^Я смотрю на заполненный белым дымом гроб посреди комнаты. Голова по-прежнему раскалывается после измытывающего сна.
