...
es.room {
    ...
    ^Берега совсем не видно. Течение несёт меня навстречу закату, точно в космическую туманность. До невозможности огромная Луна осеняет холодным светом небо. Звёзды плывут в этом свечении, как тающие угольки. Или же это их отражения играют в воде?
    ^Я не знаю, кто я.
...
es.obj {
    ...
    used = function(s, w)
        if w.nam == "paddle" then
            ...
            ^На секунду замираю, любуясь её светом.
...
es.obj {
    ...
    act = function(s)
        if not s.actdoneed then
            ...
            ^Через секунду до меня доходит -- это черви! Всё озеро кишит червями! Их тысячи, десятки тысяч. Я плыву по умирающему организму, который заполнили мерзкие паразиты.
...
es.room {
    ...
    ^К тесноте каюты привыкнуть сложно. Я бьюсь спросонья о потолок головой, хотя бедный череп и так едва не пошёл трещинами.]],
    obj = { "transition" }
}
