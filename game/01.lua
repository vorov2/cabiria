...
es.room {
    ...
    onenter = function(s)
        es.loopMusic("unseen_presence", 2000)
    end,
    onexit = function(s, t)
        if t.nam == "main" then
            p "Вернуться куда?"
...
es.obj {
    nam = "fish",
    dsc = "На секунду мне кажется, что в воде {что-то} движется. Не поднимаясь к поверхности, без единого звука, скользит под килем длинная, словно росчерк тушью, тень.",
...
es.obj {
    ...
    cnd = "not s.done",
    dsc = "Неподалеку покачивается в воде {светящаяся сфера} -- почти такая же яркая, как Луна на опустошённом небе. У меня даже мелькает мысль, что сфера родилась из отражения на волнах.",
    act = function(s)
        s.done = true
        return "Сфера совсем близко, лёгкие волны подгоняют её ко мне, она словно тянется ко моей лодке, к теплу моих рук. Я поднимаю сферу, держу её какое-то время на ладони, невесомую, отлитую из лунного света, как вдруг она резко гаснет -- и рассеивается в темноте."
    end,
    used = function(s, w)
        if w.nam == "paddle" then
...
es.room {
    ...
    onexit = function(s, t)
        if t.nam == "main" then
            p "Я чувствую, что не смогу. Стоит мне развернуть лодку, как меня тут же поглотит тёмная вода."
            return false
        end
...
es.obj {
    nam = "ghosts",
    dsc = "Я снова замечаю в воде какое-то смутное {движение} -- будто бы в недрах озера кипит неведомая жизнь.",
...
es.obj {
    ...
    cnd = "not s.done",
    dsc = "Ещё одна {сфера} невесомо скользит по тёмной воде. Кажется, её уносит от меня ревнивый ветер.",
    act = "На сей раз сфера слишком далеко, я боюсь, что не смог до неё дотянуться.",
    used = function(s, w)
        if w.nam == "paddle" then
            s.done = true
            return [[К счастью, длины весла хватает. Я подталкиваю сферу поближе к лодке и вытаскиваю её из воды.
            ^На секунду замираю, любясь её светом. Эта сфера ярче, сильнее предыдущей, но её свет тоже уходит, развеивается в дыхании озера, как пепел.]]
        end
    end
}
-- endregion
...
es.obj {
    nam = "seaweed",
    acted = false,
    dsc = "Кажется, что весло постоянно задевает за {что-то}, словно я плыву сквозь густые подводные заросли.",
    act = function(s)
        if not s.acted then
            s.acted = true
            p "Я приглядываюсь. Света Луны не хватает, но теперь мне и правда удаётся различить в воде странное движение. Рыбы? Нет. Что-то тонкое и длинное, как стебли водорослей, бьётся в подводных течениях."
        else
...
es.obj {
    ...
    cnd = "not s.done",
    dsc = "Здесь есть ещё одна налитая светом {сфера}. Интересно, кто разбросал их по всему озеру? Зачем я их собираю?",
    act = "Сфера слишком далеко от лодки, просто так мне до неё не дотянуться.",
    used = function(s, w)
        if w.nam == "paddle" and not s.nearby then
            s.nearby = true
            return "Я почти подцепляю сферу веслом, но лодка начинает раскачиваться, и вытянуть её из темной воды у меня не получается. Сфера всё ещё слишком далеко."
...
es.room {
    ...
    disp = "Озеро",
    pic = "dream/lake.boat2",
    dsc = [[Я совсем вымотался из сил, но ничего вокруг так и не изменилось. Грести стало ещё тяжелее, весло путается в каких-то подводных корнях. Я едва могу поднимать его над водой. Луна светит в глаза, точно издеваясь. Она слепит, хотя вокруг темно.]],
    onexit = function(s, t)
        if t.nam == "main" then
            p "Это сложно объяснить, но я откуда-то знаю, что не смогу вернуться -- словно всё, что осталось у меня за спиной, вычеркнуто из мироздания."
            return false
...
es.obj {
    ...
    act = function(s)
        if not s.actdoneed then
            ...
            ^Нужно продолжать плыть. Мне нужно добраться до берега.]]
        else
...
es.obj {
    ...
    used = function(s, w)
        if w.nam == "paddle" and not s.nearby then
            s.nearby = true
            return "Сфера слишком далеко, и я не могу дотянуться до неё веслом. На сей раз волны от лодки утаскивают её всё дальше в темноту. Я ложусь на дно, перегибаюсь через и, едва удерживая обоими руками огрузлое весло, пытаюсь поймать уплывающую сферу."
        elseif w.nam == "paddle" then
            s.done = true
            drop("paddle")
            walkin("pause2")
...
es.room {
    ...
    В этот момент что-то бьёт лодку в днище, поднимаясь из глубины, и лодка переворачивается.]],
    next = "deep2"
}
-- endregion
...
es.room {
    ...
    dsc = [[Я хватаюсь за весло как утопающий, который готов цепляться за любую возможность спастись, крепко, как в предсмертной судороге, вцепляюсь в рукоятку -- и медленно погружаюсь на дно вместо с веслом.
    ^Черви бешено кружатся вокруг меня, сжимают кольцами мои руки и ноги. Воздух в лёгких кончается. Вода становится тяжёлой и плотной, наваливаясь мне на грудь и плечи. Черви лезут в ноздри, в уши, в глаза.
...
es.room {
    ...
    ^Не знаю ни одного человека, который не мучился бы от кошмаров. Cам я привык к ним настолько, что перестал запоминать, однако заполненное червями озеро мне приснилось впервые.
...
es.room {
    ...
    ^К тесноте каюты привыкнуть сложно. Я бьюсь спросони о потолок головой, хотя бедный череп и так словно пошёл трещинами. Конечно, одной мигрени мне мало.]],
    obj = { "transition" }
}
