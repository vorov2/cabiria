...
es.room {
    ...
    pic = "station/block_a",
    disp = "Жилой блок А",
    dsc = [[Я до последнего верил в непричастность своего экипажа к тому кошмару, который творится на станции. Мы могли бы найти что-нибудь с Верой -- какую-нибудь улику, указывающую на обезумевшего Марутяна, решившего потравить своих коллег. А в итоге виновником оказался кофе, который мы привезли на "Грозном".]],
    onexit = function(s, t)
        if t.nam == "main" then
...
es.obj {
    ...
    dsc = "Я стою жилом блоке напротив двери в модуль Майрова. Нужно лишь протянуть руку и нажать на {кнопку} звонка.",
    act = function(s)
        if not s.done then
            s.done = true
...
es.obj {
    ...
    act = "Глядя на него сейчас, сложно представить, что ему ещё нет сорока. Капитан стал похож на измученного старика.",
    used = function(s, w)
...
es.obj {
    ...
    act = function(s)
        ...
        return "Я вслепую нащупываю кнопку и с трудом нажимаю её -- она словно сопротивляется мне, не хочет выпускать наружу."
    end
}
-- endregion
...
es.room {
    ...
    onexit = function(s, t)
        if t.nam == "deck" and not s.done then
            p "Я вдруг думаю -- а когда я сам последний раз пил кофе? Всё ли в порядке у меня с головой? Может, никакого разговора с Майоровым не было -- как и рентгеновского снимка."
            return false
        end
    end,
...
es.obj {
    ...
    act = function(s)
        all.neardeck.done = true
        return "Я совершенно не хочу вспоминать о том, что тогда произошло."
    end
}
...
es.room {
    ...
    pic = "station/deck",
    disp = "Рубка",
    dsc = [[Рубка встречает колким светом. Горят мониторы, мерцают на панелях огни. Один аварийный люминофор всё ещё расплёскивает по стенам кровавые отблески и бьёт по глазам, словно заставляет уйти.]],
    onexit = function(s, t)
...
es.obj {
    nam = "grigoriev",
    head = false,
    dsc = "{Григорьев}, как в какой-то сюрреалистической постановке, сидит перед терминалом и медленно выстукивает что-то на клавиатуре, будто пытается поймать забытый ритм.",
    act = function(s)
        if not s.head then
            s.head = true
...
es.room {
    ...
    pic = "common/blind",
    disp = "Рубка",
    dsc = [[Перед глазами всё темнеет, я почти ничего не вижу -- только всполохи света. Я чувствую себя так, словно чудом выбрался из преисподней.]],
    onexit = function(s, t)
        if t.nam == "nearlab1" then
...
es.obj {
    nam = "comps",
    dsc = "Какие-то {компьютеры} ещё работают, мигают индикаторы, шуршат жёсткие диски, процессоры производят расчёты для несуществующих двигательных блоков.",
    act = function(s)
        es.walkdlg("me.step")
        return true
    end
...
es.room {
    ...
    dsc = [[Потрескивает проводка, сыплятся искры с потолка. Пол качается под ногами, как на корабле в шторм. В ушах стоит дикий звон, словно кто-то врубил тревожный сигнал прямо у меня в черепной коробке. Мысли путаются. Я делаю несколько шагов и приваливаюсь плечом к какой-то переборке.]],
    onexit = function(s, t)
        if t.nam == "main" then
...
es.obj {
    nam = "darkness2",
    cnd = "{darkness1}.done",
    dsc = "Ко мне из разверзшейся темноты, ведущей прямиком в космическое пространство, движутся какие-то длинные {тени}.",
    act = function(s)
