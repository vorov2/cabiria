...
es.obj {
    ...
    act = function(s)
        if all.snapshot4_holder.bad then
            ...
            return "Разглядеть на снимке ничего не получается разглядеть, видимо, я напутал что-то с настройками."
        elseif s.done then
...
es.obj {
    ...
    act = "Шкафчик, к частью, не заперт на ключ -- я открываю дверцу, но внутри нет ничего, кроме толстых папок с какими-то документами. Они нам сейчас без надобности."
}
...
es.obj {
    ...
    act = function(s)
        ...
        if not s.done then
            s.done = true
            txt = txt .. "^Маркера в лотке, впрочем, нет -- вместо него там валяется круглый магнит."
        end
        return txt
    end,
    used = function(s, w)
