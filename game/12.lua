...
es.room {
    ...
    disp = "Коридор",
    dsc = [[Здесь спокойно и тихо, как и должно быть на станции, где уставшие друг от друга люди заперлись в модулях с глухими дверями. Легко представить, что ничего не произошло, что мы не падаем в атмосферу газового гиганта.]],
    onexit = function(s, t)
        if t.nam == "main" then
            p [[Лучше подождать Веру здесь. Сейчас многие на станции относятся к членам команды "Грозного" напряжённо.]]
            return false
        end
    end,
...
es.obj {
    ...
    cnd = "{grigoriev}.done",
    dsc = "{Вера} и",
    act = "Бинт на щеке ей наконец заменили. Я смотрю на Веру и чувствую себя так, словно это именно я виноват в том, что произошло."
}
...
es.obj {
    ...
    act = function(s)
        s.done = true
        return "Из конца коридора доносятся чьи-то обеспокоенные голоса. Вскоре в бледном свету уставших ламп вырисовывается человеческая фигура. Я не сразу узнаю Григорьева."
    end
}
...
es.room {
    ...
    dsc = [[Реанимационная неожиданно напоминает мне отсек навигации на "Грозном" -- даже освещение и собирающиеся по углам, как морщины, тени. Пользуются этим отсеком наверняка не слишком часто -- в воздухе витает поблёскивающая пыль, которую гоняют ожившие с нашим приходом воздуховоды.]],
    onexit = function(s, t)
...
es.obj {
    ...
    cnd = "not {xray_snapshot}.done",
    dsc = "Мерцающий, точно в истерическом припадке, {монитор} явно доживает последние дни.",
    act = function(s)
        if s.done then
...
es.obj {
    ...
    act = function(s)
        if not s.light then
            s.light = true
            return "Я щёлкаю переключателем, и панель для просвечивания рентгеновских снимков наливается светом."
        else
            s.light = false
...
es.obj {
    ...
    act = function(s)
        if all.snapshot4_holder.bad then
            ...
            return "Разглядеть на снимке ничего не получается, видимо, я напутал что-то с настройками."
        elseif s.done then
...
es.obj {
    ...
    act = "Шкафчик, к частью, не заперт на ключ -- я открываю дверцы, но внутри нет ничего, кроме толстых папок с какими-то документами. Они нам сейчас без надобности."
}

es.obj {
    nam = "capsule",
    done = false,
    dsc = "Чуть дальше, ближе к двери в соседний отсек, стоит {медицинская капсула}, из-за которой сходство реанимационной с отсеком навигации становится даже немного пугающим.",
    act = function(s)
        if s.done then
            ...
            s.done = true
            return [[Наполовину сделанный из стекла саркофаг траурно затянут тенью -- нужно подойти и коснуться крышки ладонью, чтобы она стала прозрачной.
            ^Несколько секунд я просто стою и смотрю на капсулу, как будто хочу убедиться в том, что она мне не мерещится. Потом кладу на неё руку. Тень мгновенно спадает, как погребальный саван, но смотреть на тело навигатора я почему-то не могу.
...
es.obj {
    ...
    act = function(s)
        if not all.vera.xray or all.xray_snapshot.done then
            return "Нет времени возиться с этим терминалом, у меня есть другие дела."
        else
            s.unlocked = false
...
es.terminal {
    ...
    commands = {
        ...
        contrast = function(s, args)
            ...
            if not lvl then 
                return {
                    ...
                    "Допустимые значения уровня контраста: от 0 до 20"
                }
            elseif lvl < 0 or lvl > 20 then
                return {
        ...
        xray = function(s, args, load)
            ...
            all.snapshot4_holder.bad = (s.vars.contrast ~= 20 or lux ~= s.vars.lux)
...
es.room {
    ...
    dsc = [[Второй отсек реанимационной похож на основной -- и также насквозь пропитан пылью. Потоки воздуха гоняют её над головой, и кажется, что я оказался в забытой кладовке.]],
    preact = function(s)
        if not s.mus and not snd.music_playing() then
            s.mus = true
            es.music("fatigue2", 2)
...
es.obj {
    ...
    dsc = function(s)
        if all.pincer_holder.taken and all.cup_holder.taken then
            return "Только на привалившемся к стенке столе вместо ВА стоит жестяной {короб}."
        else
            return "Только на привалившемся к стенке столе вместо ВА стоит жестяной {короб}, где"
        end
    end,
...
es.obj {
    ...
    act = function(s)
        ...
        if not s.done then
            s.done = true
            txt = txt .. "^Маркера в лотке, впрочем, нет -- вместо него там валяется круглый магнитик."
        end
        return txt
    end,
    used = function(s, w)
...
es.obj {
    ...
    act = function(s)
        if not s.broken then
            s.broken = true
            return "Я пытаюсь взять магнит, тот в ответ трещит, и в руках у меня оказывается лишь яркая пластиковая облатка."
        else
...
es.room {
    ...
    disp = "Коридор",
    dsc = [[Я совсем забыл об усталости и иду быстрым размашистым шагом, но привычный уже коридор стал как будто длиннее, словно всё пространство вокруг искажается, пока падаем в Сантори-5.]],
    enter = function(s)
        if not s.mus then
            s.mus = true
            es.music("bass")
...
es.obj {
    nam = "pair",
    dsc = [[У стены, под падающим с потолка светом, точно под софитом, стоят {мужчина и женщина}. Женщина закрыла ладонями лицо и беззвучно плачет. Мужчина говорит ей что-то шёпотом, поглаживая по плечу.]],
    act = "Мне кажется, лучше им не мешать."
}
-- endregion
...
es.obj {
    nam = "walls",
    dsc = "По стенам проходит мелкая вибрация, точно дрожь умирающего механизма.",
...
es.room {
    ...
    pic = "station/diner",
    disp = "Столовая",
    dsc = [[Я наконец решаюсь и захожу в столовую. Первое, что я чувствую -- резкий и горклый запах кофе.]],
    enter = function(s)
        if not s.mus then
            s.mus = true
            es.music("countdown", 1, 0, 5000)
        end
    end,
    onexit = function(s, t)
        if t.nam == "diner_corridor2" and not all.efimova.done then
            es.walkdlg("efimova.main")
            return false
...
es.room {
    ...
    dsc = [[Автомат похож на промышленный холодильник со светящимися молочно-белыми кнопками, на которые написаны названия напитков и блюд.]],
    obj = { "coffeebutton" },
    way = {
        path { "Отойти", "diner" }
    }
...
es.obj {
    nam = "coffeebutton",
    dsc = "В глазах у меня двоится от волнения, удары сердца, точно эхом, отдаются в виски. Я не сразу нахожу кнопку с пометкой \"{Кофе}\".",
    act = function(s)
