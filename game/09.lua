...
es.obj {
    ...
    dsc = "Тёмные {пятна крови} тянутся по полу к двери в модуль Марутяна.",
...
es.obj {
    nam = "cabin_door",
    blocked = true,
    dsc = "Пятна тянутся дорожкой к {двери} в модуль Марутяня.",
    act = function(s)
        if s.blocked then
...
es.room {
    ...
    ^^Я должен возвращаться, надо позвонить по коммуникатору.]],
    enter = function(s)
        if not s.mus then
            s.mus = true
            es.music("hemisphere")
...
es.obj {
    ...
    dsc = [[У входа стоят {несколько человек}, которых я уже видел в шлюзе после стыковки, серых и безликих. Они, видимо, материализуются на станции во время чрезвычайных ситуаций. Одно я даже узнаю -- это Михаил Лысанов, помощник Мицюкина.]],
    act = function(s)
        if not s.done then
            s.done = true
            es.walkdlg("lysanov.head")
...
es.obj {
    ...
    act = "Можно подумать, их здесь оставили здесь нарочно, как напоминание."
}
-- endregion
...
es.obj {
    ...
    act = function(s)
        if not s.locked then
            s.locked = true
            return "У меня нет сил больше любоваться космическими видами, от близости странной планеты идёт кругом голова. Я щёлкаю кнопкой на стене, и шторка иллюминатора закрывается, шелестя металлическими пластинами."
        else
            s.locked = false
...
es.obj {
    ...
    dsc = "У мутного {снимка}, видно, сделанного сквозь иллюминатор, замяты уголки, а сгиб посередине протёрся так, что космический вид дробит попалам белая полоса, словно выеденная червями.",
    act = function(s)
        if s.done then
            return "Я не сохранил никаких фотографий своего первого корабля."
        else
            s.done = true
            es.walkdlg {
                dlg = "mercel",
...
es.room {
    ...
    pic = "station/toilet1",
    disp = "Санузел",
    dsc = [[Лампа в саунзеле мерцает и раздражающе трещит. Интересно, почему Мерцель не попросила, чтобы ей заменили лампу?]],
...
es.room {
    ...
    pic = "station/cabin4",
    disp = "Жилой модуль B3",
    dsc = [[Я подхожу к двери и думаю, что надо было сразу идти в лабораторию. Там бы перестала болеть голова, и всё стало сразу бы понятно, мы бы не путались в словах.
