...
es.obj {
    ...
    dsc = function(s)
        if not all.belt1.done then
            ...
            return "Я {завис} посреди рубки, пытаюясь собраться с мыслями."
        end
    end,
    act = function(s)
        if not all.belt1.done then
            return "Я даже не помню как заснул -- и почему залез в ложемент Григорьева, когда у меня есть свой? Боюсь, как бы мне не влепили выговор. Отличная вахта получилась. Но силы у меня и правда на исходе -- больше в долгий дрейф я не пойду."
        else
...
es.obj {
    ...
    cnd = "not s.done",
    dsc = "{ремень} безопасности стискивает мне грудь.",
    act = function(s)
        s.done = true
        es.music("fatigue", 3)
...
obj {
    ...
    act = [[У Григорьева своеобразное чувство юмора. В действительности, пока мы идём в дрейфе, увидеть что-то в иллюминатор невозможно.
    ^Мы плывём за пределами обычного пространства и времени, в измерении, которое человеческий мозг не в состоянии помыслить, если только ты не находишься в кареологической фуге.]]
}
-- endregion
...
es.room {
    ...
    pic = "ship/deck",
    disp = "Рубка",
    dsc = [[Я понимаю, как все мы намучились за этот полёт -- больше четырехсот часов изнуряющего безделья посреди пустоты, который обычный человеческий мозг не в состоянии осмыслить. Даже Майоров, опытный капитан, похож на коматозника, которому всё приходится делать через силу.
...
es.obj {
    ...
    dsc = function(s)
        if s.done then
            ...
            return "Последний пустой {ложемент} у двери напоминает почему-то больничную койку."
        end
    end,
    act = function(s)
        if not s.done then
...
es.room {
    ...
    pic = "ship/deck",
    disp = "Рубка",
    dsc = [[-- Всех поздравляю, -- говорит Майоров. -- Вереснев! -- Он поворачивается ко мне. -- Врубай компы.
...
es.obj {
    ...
    dsc = "На мгновение мне кажется, что корабль начинает покачиваться, как на волнах, и я судорожно цепляюсь за леер. В виски покалывает -- {мигрень} так и не прошла.",
    act = function(s)
        if not s.done then
...
es.room {
    ...
    pic = "ship/cabin",
    disp = "Каюта",
    dsc = [[Я должен торопиться, но вместо этого я зачем-то залезаю в свою каюту, в эти несколько кубометров личного пространства, как будто именно сейчас, перед стыковкой, мне нужно спрятаться, побыть несколько секунд одному.]],
    onexit = function(s, t)
        if t.nam == "main" then
...
es.obj {
    ...
    act = function(s)
        all.pain.done = true
        return [[Таблеток от мигрени больше не осталось -- видимо, весь экипаж подсел на них во время полёта в червоточине. Надо будет пополнить запасы -- мы должны везти много медикаментом в грузовых отсеках.
...
es.obj {
    ...
    inv = "Реаниматор -- это здоровый чёрный диск, похожий на противопехотную мину, который заводится по часовой стрелки."
}
-- endregion
...
es.room {
    ...
    timer = function(s)
        ...
        elseif all.gravs.wait == 40 then
            ...
            all.gravs.done = true
            all.gravs.wait = 0
            p [[Гравитация обрушивается на меня свинцовой волной. Я падаю на колени, не отпуская леер, как будто от него зависит моя жизнь.
            ^Требуется какое-то время, чтобы свыкнуться с вновь обретённой силой тяжести. Несмотря на то, что искусственной гравитации на "Грозном" далеко до земной, мышцы ноют от боли, требуя пощады.]]
            return true
        else
            return false
        end
...
es.obj {
    ...
    act = function(s)
        if s.unbar then
            ...
            return "Для доступа к распределительному щиту нужен ключ."
        end
    end,
    used = function(s, w)
        ...
        elseif w.nam == "patron" then
            s.patron = true
            purge("patron")
            return "Я вставляю в отверстие предохранительный патрон, поворачиваю его по часовой стрелке, и он сам, медленно раскручиваясь против бега секунд, втягивается в панель."
...
es.obj {
    ...
    used = function(s, w)
        ...
        elseif w.nam == "key" and s.wait == 0 then
            s.wait = 1
            return [[Больше четырёхсот часов без гравитации -- тело уже отвыкло от того, что у него есть вес. Не просто так меня посылают реанимировать корабль после червоточины -- как самого молодого. Сами-то лежат в своих ложементах, опутанные ремнями.
            ^Я вставляю ключ в разъем и поворачиваю до упора. Из машины раздаётся нарастающий гул. Надо повернуть ключ ещё один раз, и тогда у меня останется несколько секунд перед тем, как заработают гравитационные катушки.]]
        elseif w.nam == "key" and s.wait == 1 then
            s.wait = 2
            timer:set(1000)
            es.stopMusic()
            es.loopMusic("descend")
            return "Я резко крутанул ключ и вцепился в леер на стене, как страховочный фал. Всё, теперь надо просто ждать."
        elseif w.nam == "key" and s.wait > 1 then
...
es.obj {
    ...
    act = function(s)
        if not all.majorov2.talk then
            return "Я подхожу к капсулу и касаюсь стеклянного забрала -- туман внутри проясняется, и я вижу лицо навигатора, которое кажется таким же мертвым, как и всегда."
        else
...
es.obj {
    ...
    cnd = "not s.done",
    dsc = "Я {расстёгиваю} на груди девушки её тонкую белую форму, похожую на больничную робу.",
    act = function(s)
        if not s.try then
            s.try = true
...
es.obj {
    ...
    dsc = "Я {ставлю} реаниматор на грудь девушки. Эта штука кажется не медицинским устройством, а каким-то страшным пыточным инструментом.",
    act = function(s)
        s.done = true
        walkin("interlude5")
        return true
...
es.obj {
    ...
    dsc = "Я {поворачиваю} по часовой стрелке тугой трещащий барабан реаниматора, словно завожу сломанную куклу.",
    act = function(s)
        s.done = true
        walkin("interlude6")
        return true
...
es.obj {
    ...
    dsc = "Барабан медленно вращается против часовой стрелке, громко отсчитывая секунды. Девушка несколько раз вздрагивает. Майоров проверяет её пульс и качает головой. Но можно {попробовать} снова!",
    act = function(s)
        s.done = true
        walkin("interlude7")
        return true
...
es.room {
    ...
    ^^В **** году один из экспериментов привёл к смерти человека. На внеочередном съезде партийный деки было принято решение прекратить все эксперименты.
