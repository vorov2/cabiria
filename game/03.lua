...
es.obj {
    ...
    inv = "Реаниматор -- это здоровый чёрный диск, похожий на противопехотную мину, который заводится по часовой стрелке."
}
-- endregion
...
es.obj {
    ...
    used = function(s, w)
        ...
        elseif w.nam == "key" and s.wait == 0 then
            ...
            ^Я вставляю ключ в разъем и поворачиваю до упора. Из машины раздаётся нарастающий гул. Надо повернуть ещё один разок, и тогда у меня останется несколько секунд перед тем, как заработают гравитационные катушки.]]
...
es.obj {
    ...
    dsc = "Барабан медленно вращается против часовой стрелки, методично отсчитывая секунды, как бомба замедленного действия. Девушка несколько раз вздрагивает. Майоров проверяет её пульс и качает головой. Но можно {попробовать} снова!",
    act = function(s)
        s.done = true
        walkin("interlude7")
        return true
