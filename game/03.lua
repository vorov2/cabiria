...
es.obj {
    ...
    act = function(s)
        ...
        local poses = {
            ...
            "Ложемент раскладывается, как кровать, и становится почти плоским. Что-то похрустывает у меня в пояснице.",
        ...
        local checks = {
            ...
            ^Мерцель тоже пожужжала ложементом (остальные, видимо, не восприняли указания капитана всерьёз), и на нас снова навалилась тишина.
            ^Казалось, прошло несколько часов, прежде чем под потолком загорелся первый алый люминофор.
            ^Выход начался.]]
        }
        if s.position == 3 then
            s.position = 0
...
es.room {
    ...
    ^^В **** году после прихода нового генерального секретаря развитие исследования дальних рубежей космоса было признано приоритетным направлением. Работа над кареалогической фугой возобновилась. В экспериментах принимали участие только женщины. Семьи участников получали государственные дотации на пятьдесят лет.]],
    next = "nav3"
}
-- endregion
