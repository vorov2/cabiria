...
es.room {
    ...
    dsc = [[Навигационный отсек кажется камерой для чудовищных экспериментов -- особенно сама капсула, стоящая на металлическом пьедестале, из которого выплетаются толстые гофрированные трубы.]],
    onexit = function(s, t)
        if t.nam == "main" then
...
es.room {
    ...
    dsc = function(s)
        if not s.done then
            ...
            return [[Каюту насквозь пронизывает резкий химозный холод. Я быстро одеваюсь. Меня снова трясёт.]]
        end
    end,
    onexit = function(s, t)
...
es.room {
    ...
    pic = "ship/deck",
    disp = "Рубка",
    dsc = [[Свет в рубке такой же яркий, как и в навигационном отсеке. Я прикрываюсь от света ладонью.]],
    obj = { "majorov3" },
    way = {
        path { "В коридор", "ship4" }
    }
