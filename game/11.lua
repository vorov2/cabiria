...
es.room {
    ...
    pic = "station/med1",
    disp = "Медотсек 01",
    dsc = [[Белые стены медотсека давят, не дают дышать. Если долго смотришь в одну точку, всё вокруг начинает рассеиваться в каком-то предобморочном шуме, и глаза заливает белизна.]],
    preact = function(s)
        if not s.mus then
            s.mus = true
...
es.obj {
    ...
    act = "Повязку у меня на голове сменили, а перед этим залили рану едким медицинским клеем, из-за чего теперь кажется, будто череп у меня расходится по швам."
}
...
es.obj {
    ...
    act = "Мне нужен хотя бы глоток воды."
}

es.obj {
    nam = "bottles",
    taken = false,
    dsc = [[На полу валяется разодранная полиэтиленовая {пачка бутылок} с простой водой, которую, видимо, притащили с "Грозного".]],
    act = function(s)
        if not s.taken then
            s.taken = true
            take("bottle")
            return "Я вытаскиваю из пачки одну бутылку."
        else
...
es.obj {
    nam = "andreev",
    dsc = "{Андреева}, которого перед этим щедро напичкали обезболивающим, и он ничего вокруг не замечает вокруг, уставившись в точку мутным взглядом.",
...
es.room {
    ...
    dsc = function(s)
        if not s.stage then
            return [[Приёмная заполнена людьми. Я как будто оказался в поликлинике в шумный рабочий день.]]
        else
            return [[В приёмной стало спокойнее, как будто в медблоке начался тихий час. Я и сам не понимаю, что ещё здесь делаю.]]
        end
    end,
    enter = function(s)
        if not s.mus then
...
es.obj {
    nam = "mitsukin",
    cnd = "not {reception}.stage",
    dsc = "и {Мицюкин} шумно спорят о чём-то у выхода в коридор, и на них раздражённо поглядывают другие посетители, но никто не решается им что-то сказать.",
...
es.obj {
    ...
    dsc = function(s)
        if not all.reception.stage then
            return "{Вера} о чём-то перешёптывается с Ефимовой у двери во второй медотсек, поддерживая её за руку."
        else
            return "Ефимова ушла, и {Вера} расхаживает у двери во второй медотсек, чего-то ожидая."
        end
    end,
    act = function(s)
        if not all.reception.stage then
...
es.obj {
    nam = "efimova",
    dsc = "{Ефимова} внимательно наблюдает на скачущими показаниями на экране.",
    act = function(s)
...
es.room {
    ...
    ^Я бреду по коридору, как зомби. Вера дёргает меня за рукав.]],
    enter = function(s)
...
es.room {
    ...
    pic = "station/chem1",
    disp = "Химическая лаборатория",
    dsc = [[Помещение лаборатории ненамного больше личных модулей, и здесь тесно даже вдвоём. Дышать почти нечем, хотя воздуховоды работают, и мою шею холодит лёгкий ветерок. Возможно, во всём виноват стойкий и едкий запах, словно здесь что-то растворяли в кислоте. Не представляю, как можно работать в такой обстановке весь день, а Вера провела здесь уже больше двух лет.]],
    onexit = function(s, t)
...
es.obj {
    ...
    inv = "Именная ключ-карта на Андрея Верховенцева."
}
...
es.obj {
    ...
    act = function(s)
        if s.done then
            return "Кажется, она не хочет об этом говорить."
        end
        s.done = true
        es.stopMusic(3000)
...
es.room {
    ...
    dsc = [[Этот отсек ещё меньше предыдущего. Можно подумать, что на химиках здесь нарочно экономят места. За освещение к тому же отвечает одна-единственная пыльная лампа.]],
