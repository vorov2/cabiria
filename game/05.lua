...
es.room {
    ...
    ^К Майорову тут же подбежала девушка в светлой униформе. Меня сторонились, как заразного. Может быть, вышедшую нам навстречу делегацию пугал мой скафандр.]],
    obj = { "takeoff_helmet" }
}
...
es.obj {
    ...
    act = "Непонятно, что я здесь делаю. Андреев дал мне ключ-карту. Я устал и хочу спать.",
    used = function(s, w)
        if w.nam == "card" then
            walkin("c1a")
            return true
...
es.obj {
    ...
    tak = function(s)
        if not s.examed then
            s.examed = true
            p "Я аккуратно, чтобы не упасть, перегибаюсь через борт. В глаза тут же бьёт яркий живой свет, я тянусь к нему, но маленькая сфера проворно, как живое существо, отскакивает от лодки и, проскользив над самой поверхностью воды, устремляется в темноту."
            return false
        elseif s.examed and not s.near then
...
es.obj {
    ...
    used = function(s, w)
        ...
        elseif w.nam == "paddle" and not s.examed then
            ...
            ^Меня так зачаровывают эти живые тени, что сфера едва не выскальзывает из сетки на весле. Что бы со мной произошло, если бы я остался без света! Надо быть осторожнее.]]
        end
    end
}
-- endregion
...
es.room {
    ...
    dsc = [[Я делаю несколько сильных гребков, и темнота впереди расступается, пропускает меня, испуганная светом. Чёрные извивающиеся полосы проносятся над головой, едва не касаясь щёк. Сфера раскачивается в сетке на флагштоке, как от ветра.]],
    next = function(s)
        es.music("santorum")
        walkin("wake2")
    end
...
es.room {
    ...
    ^Темнота тут же набрасывается на меня, вмиг смывая последние отблески света. Больше нет ни Луны, ни озера, ни лодки. Но я не в пустоте. Тьма вокруг наполнена движением, жизнью, ликующей из-за того, что избавилась от света.
