...
es.room {
    ...
    pic = "station/dock",
    disp = "Пирс",
    dsc = [[У пирса наш ожидала аварийная команда -- несколько человек в серых, похожих на рабочие робы, комбинезонах.
    ...
    ^Он ловко вынырнул из люка, распластался на полу, придавленный гравитацией, но быстро встал на ноги, лишь едва заметно покачнувшись. Волосы -- слиплись от пота. Кожа на лице разошлась пунцовыми пятнами, как от проказы.
...
es.room {
    ...
    noinv = true,
    pic = "station/dock",
    dsc = [[Разглувать здесь в тесном и увесистом скафандре не так-то просто -- на корабле он тоже не казался лёгким, а сейчас нещадно вяжет свинцовой тяжестью шаги, точно я взгромоздил на себя водолазный костюм начала 19-го века.]],
    next = "dock"
}
-- endregion
...
es.room {
    ...
    next = function(s)
        ...
        p "Я вернулся на корабль и с удовольствием скинул с себя тяжёлый скафандр. Кажется, стало даже легче дышать."
        walkin("dock")
    end
}
-- endregion
...
es.room {
    ...
    dsc = [[От нарастающего шума, звучащих вразнобой голосов, кружится голова. Коридор у пристани кажется невыносимо огромным. За две с лишним недели на "Грозном" я так привык к дефициту пространства, что чувствую себя, как глубоководная рыба, которую вытащили на поверхность.]],
    obj = {
        "otherdock",
        "majorov",
        "girl"
...
es.room {
    ...
    disp = "Коридор у пристани",
    dsc = [[Знаменитая "Кабирия" выглядит точно так же, как и все известные мне орбитальные станции -- грубые металлические переборки, энергосберегающие лампы, разливающие по стенам стерильный свет, жёлтые сигнальные люминофоры, которые зажигаются, когда у пирса стоит корабль...]],
    enter = function(s)
...
es.room {
    ...
    dsc = [[Свет приглушили, и коридор у пирса охватил вязкий, едва разреженный тусклыми лампами полумрак, напоминающий о сумерках на Земле. Я подумал -- а когда мы вернёмся домой? Здесь, на орбите Сантори-5, Земля кажется далёкой, как сон, словно во сне только и существует.]],
    next = function(s)
...
es.obj {
    nam = "hands",
    cnd = "not have('paddle')",
    dsc = "и собственные {руки}, которые замерли в густом сумраке, как у подвешенной на нитки марионетки.",
...
es.obj {
    ...
    used = function(s, w)
        ...
        elseif w.nam == "paddle" and not s.examed then
            s.examed = true
            return [[Я аккуратно поднимаю весло над водой -- так, чтобы не вывалилась сфера, -- приглядываюсь и вижу, как тонкие чёрные ленты вылезают из омута, сплетаясь в плотную стену, которая не даёт дальше.
...
es.room {
    ...
    ^День 3865, 07:22 у.в.
    ^^Вереснев Олег Викторович, техник ГМК "Грозный", 26 полных лет, был обнаружен сегодня мёртвым в своём жилом модуле. Причина смерти -- кровоизлияние в мозг. Предрасположенностей для кровоизлияния не имел. Проводится детальный химический анализ крови.]],
    enter = function(s)
        es.music("death")
    end,
    next_disp = "RELOAD",
