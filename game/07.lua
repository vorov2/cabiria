...
es.obj {
    nam = "clock",
    template = "НИОС \"Кабирия\"^День третий^^08:12:%s",
    dsc = function(s)
...
es.obj {
    ...
    act = "Впрочем, мне пока нужно только выспаться."
}
...
es.obj {
    ...
    opened = false,
    dsc = function(s)
        local txt = "Если бы не прямоугольный {иллюминатор}, личное окно на орбиту Сантори-5, я бы и правда чувствовал себя так, словно меня заперли в жестяной клетке."
        if s.opened then
    ...
    act = function(s)
        if s.opened then
            ...
            s.opened = true
            return "Я нажимаю кнопку на стене, шторма иллюминатора опускается, и взгляд упирается в пустоту -- в это время ничего не видно, ни планеты, ни звёзд, лишь муаровая темнота космического пространства."
        end
    end
}
...
obj {
    nam = "knock",
    dsc = "Со стороны двери раздаётся надрывная монотонная {мелодия}, похожая какой-то технический шум. Я не сразу понимаю, что ко мне пришли гости.",
    act = function(s)
        es.stopMusic(4000)
        es.walkdlg {
            dlg = "marytan",
...
es.room {
    ...
    disp = "Коридор",
    dsc = [[В коридорах между жилыми блоками куда больше света, чем на нижних палубах, где находятся пирсы.Я невольно осматриваюсь.]],
    onenter = function(s)
        es.music("hope")
    end,
...
es.obj {
    nam = "talk",
    dsc = "{Марутян} оживлённо разговаривет с Симоновой, которая сосредоточенно морщится и кивает, глядя на него так, словно ей не терпится поскорее отправиться по своим делам, но перебивать руководителя она решается.",
    act = function(s)
...
es.obj {
    ...
    act = function(s)
        if not s.sit then
            s.sit = true
            return "Мой новый коллег наконец что-то выбирает и, удовлетворённо улыбаясь, отправляется к столику с подносом."
        else
...
local option_base = {
    ...
    used = function(s, w)
        if w.nam == "key" then
            ...
            purge("key")
            take("breakfast")
            local buttons = { "первую", "вторую", "третью" }
            return string.format(
                [[Я несколько раз поворачиваю %s ключ против часовой стрелки, словно завожу внутри продуктового аппарата шестерёнчатый механизм. Ключ с каждым оборотам движется всё тяжелее, пока наконец не замирает, до предела натянув невидимую пружину.
                ^Аппарат дрожит. В его недрах что-то металлически позвякивает, даже светящаяся надпись "Завтрак" на секунду мигает. Раздаётся бодрый звонок, загорается надпись "Приятного аппетита" -- как бы опережая события, как будто внутри механизма что-то разладилось, -- и на открывшемся лотке появляется пластиковый контейнер.]],
                buttons[s.idx])
        end
    end
}
...
es.obj(option_base) {
    nam = "option1",
    idx = 1,
    dsc = [[Лицевая панель устройства раздела на четыре секции, первые три из которых соответствуют тому или иному приёму пищи, а последняя -- напитка. Подсвечивается только "Завтрак" с напитками. Выбор на завтрак -- самый скудный. Под затёртой выштамповкой "комплексное питание" размещены серенькие таблички с прорезями для винтового ключа (завтрак {№1},]],
...
es.obj {
    ...
    dsc = "Бледно-розовый компот в {пластиковом стаканчике} напоминает какую-то техническую жидкость.",
    act = function(s)
        s.drink = s.drink + 1
        if s.drink == 1 then
            return "Я делаю несколько глотков компота. Он на удивление сладкий, от вяжущей приторности сводит рот, точно я взял не компот, а чистый раствор сахарного сиропа."
        elseif s.drink == 5 then
            return "Я допиваю приторную жижу, которую здесь называют компотом."
        else
            return "Я делаю ещё один глоток."
        end
    end
}
...
es.room {
    ...
    dsc = [[Мы поднимаемся на самом медленном на моей памяти лифте на второй уровень станции, где находится лаборатория Марутяна.
...
es.room {
    ...
    pic = "station/lab",
    disp = "Лаборатория",
    dsc = [[Лаборатория напоминает проявочную в фотоателье, тонущую в треске вычислительных аппаратов. Всё вокруг затягивает плотный красный свет, выжигающий роговицу.]],
    enter = function(s)
        if not s.mus then
            s.mus = true
...
es.obj {
    nam = "marytan4",
    dsc = "{Марутян} стоит, сцепив руки на груди, и хитро посматривает на меня, сдвинув кустистые брови.",
    act = function(s)
...
es.room {
    ...
    effects = {
        ["1x0"] = nil,
        ["2x0"] = "Нуболиды как-то необычно сгруппировались, как будто аквариум стал меньше и напоминают теперь плотные заросли ламинариев.",
        ["3x0"] = "Нуболиды яростно сплелились друг с другом, образовав какое-то подобие извилистого ствола, прорастающиего по центру аквариума.Глядя на них, кажется, что это -- лишённая кожного покрова конечность, представляющая собой судорожное переплетние мышц и жил.",
        ["0x1"] = nil,
        ["1x1"] = "Кажется, нуболидов раскачивают невидимые волны, всё сильнее и сильнее, пока они вдруг не замирают на едва ощутимое мгновение -- изображают стоп-кадр на видео-записи, -- и вновь продолжают свой странный танец с неровным ритмом.",
        ["2x1"] = "Часть нуболидов сжалась в маленький клубок, зависшее посреди аквариума, пока другие носятся вокруг него алыми лентами, изображая движение электронов.",
        ["3x1"] = nil,
        ["0x2"] = nil,
        ["1x2"] = "Нуболиды облепили стёкла аквариума, как будто что-то выталкивает их изнутри, и теперь до отвращения напоминают ленточных червей.",
        ["2x2"] = nil,
        ["3x2"] = "Теперь нуболиды напоминают огромное сердце, которое резко пульсирует, то сжимается в крепкий клубок, то расплетается наружу, врезаясь тонкими отростками в стекло, бьётся о стенки, чтобы через мгновение вновь собраться воедино.",
        ["0x3"] = "Нуболиды бешено носятся в аквариуме, закручиваясь спиралью. Можно подумать, что их количество увеличилось, как будто они размножаются под воздействием ионизации.",
        ["1x3"] = "Нуболиды носятся по аквариуму, иногда слетаясь вместе и образуя странные дёргающие, как при агонии конечности. Кажется, что какое-то разорванное на части создание пытается вновь собраться воедино, и никак не может.",
        ["2x3"] = nil,
        ["3x3"] = nil
    },
    enter = function(s)
...
es.room {
    ...
    disp = "Лаборатория",
    dsc = [[Марутян без слов подходит к терминалу и вставляет свою карту в приёмник.
    ...
    ^Аквариум заполняют световые блики, не имеющие источника, закручиваются в спираль, и в этот момент ткань пространства словно прорывается, и из образовавшейся червоточины вываливается мерзкая копошащаяся масса алых червей.]],
    enter = function(s)
        es.music("hemisphere")
    end,
    next_disp = "-- Что это?",
...
es.room {
    ...
    pic = "common/planet2",
    disp = "Холл",
    dsc = [[Кажется, что планету возмущают наши эксперименты над нуболидами -- её синяя атмосфера вскипает, и в густых наслоениях облаков прорезается глаз бури, напоминающий мне эхо от далёкого взрыва.]],
    enter = function(s)
        es.music("santorum")
    end,
    next = "hall2"
...
es.room {
    ...
    dsc = function(s)
        if s.done then
            ...
            return [[После смены я устал так, как будто работал, не разгибаясь, несколько дней. По часам на станции, показывающим какое-то фальшивое время, ещё ранний вечер, но всё, чего мне хочется -- вернуться в свою каюту и провалиться в сон.]]
        end
    end,
    onexit = function(s)
        s.done = true
...
es.obj {
    nam = "lonely",
    dsc = "Свет в коридоре наконец слегка приглушили, как бы намекая обитателям, что пора уже потихоньку разбредаться по клеткам. Впрочем, я никого и не вижу. Я один спустился на лифте и один бреду по {сумеречному коридору} в жилой модуль.",
...
es.obj {
    nam = "howl",
    dsc = "Из стен доносится раздражающий монотонный {гул}, который срывается на протяжное завывание, точно даже переборки станции стонут от одиночества.",
...
es.room {
    ...
    pic = "station/cabin1",
    disp = "Каюта",
    dsc = [[Заснуть я не мог. Стоило мне прилечь на кровать, как сердце начинало молотить с бешеным ритмом, а страшная темнота, напоминающая о моих непрестанных кошмарах, затягивала в себя, словно чёрный омут.
    ^Всю ночь я бродил по каюте, вспоминая о разговоре с Верой. Может, и о нас забудут в центре, не захотят отправлять за нами корабль, и мы проторчим на станции целый год.]],
    enter = function(s)
        es.music("bass", 2)
    end,
    next = "c1b"
...
es.room {
    ...
    pic = "station/cabin1",
    disp = "Жилой модуль C1",
    dsc = [[Я вспоминаю слова Веры о том, что станция стала для неё тюрьмой, и жилой модуль, который недавно казался мне огромным по сравнению с душными отсеками "Грозного", напоминает теперь тюремную камеру.]],
    onexit = function(s, t)
...
es.obj {
    ...
    dsc = function(s)
        if s.feel == 0 then
            return "{Головная боль} превратилась в жестокую пытку. Думаю, стоит завтра заглянуть в медблок. Кажется, что у меня выросла огромная пульсирующая опухоль, которая давит изнутри на черепную коробку."
        elseif s.feel == 1 then
            return "Головная боль немного отпустила, но всё ещё не проходит -- теперь я чувствую себя так, словно мне забивают иглы в виски."
        end
    end,
...
es.obj {
    nam = "outside",
    cnd = "not {glare}.done",
    dsc = "Из-за бессонницы каюта стала казаться мне спёртой, как гроб. Я едва не задеваю за стены плечами. Быть может, и головная боль разыгралась из-за этой давящей {тесноты}. Хочется выйти наружу.",
...
es.obj {
    ...
    act = function(s)
        if all.pain.feel == 2 then
            return "Я присаживаюсь на кровать и тут же вскакиваю, как полоумный. Лучше уж остаться без сна, чем вновь погружаться в этот выматывающий кошмар."
        else
...
es.obj {
    ...
    dsc = function(s)
        if not s.done then
            return "Закрытый {иллюминатор} напоминает мне окно в тюремной камере, в которое позволено смотреть лишь в избранные часы."
        end
    end,
    act = function(s)
...
es.obj {
    ...
    cnd = "{window}.done",
    dsc = "^^В иллюминатор видно планету -- её яркий до боли в глазах нимб, как при рассвете. Я даже замечаю {шторм}, на который показала мне Симонова днём.",
    act = function(s)
        if not s.done then
            s.done = true
            return "Кажется, шторм разыгрался, вихрь с слепым глазом в центре стал больше и грозится захватить теперь всю поверхность планеты. Я продолжаю приглядываться."
        else
            snapshots:make()
            walkin("pause5")
            return true
...
es.room {
    ...
    obj = { "medbox", "handwash", "mirror", "shower" },
    way = {
        path { "В каюту", "c1b" }
    }
}
...
es.obj {
    ...
    dsc = "Электрохроматическое зеркало на стене рядом с {умывальником}",
    act = function(s)
        if not s.done then
            s.done = true
            all.pain.feel = all.pain.feel + 1
...
es.obj {
    nam = "mirror",
    dsc = "отливает чернотой, пока не коснёшься его поверхности, и лишь тогда, от тепла твоих рук, в нём появляется сумрачное {отражение} -- лицо в обрамлении темноты, которое едва получается узнать.",
    act = "Можно подумать, здешние обитатели не любят смотреть на своё отражение. А по вечерам прячутся друг от друга в многочисленных отсеках."
}
...
es.room {
    ...
    pic = "station/block_c",
    disp = "Коридор",
    dsc = [[Света в коридоре почти нет, и из-за этого он кажется бесконечным. Узкие прорези выгоревших ламп, похожие на трещины в сводах, медленно, шаг за шагом, сливаются с темнотой. Перед глазами у меня тянется бесконечный коридор с угасающим светом. Я почему-то уверен, что, если пойду вперёд, то тут же провалюсь в чёрную бездну, как в моих кошмарах.]],
    obj = { "silence", "glare" },
    way = {
        path { "В каюту", "c1b" }
    }
...
es.obj {
    ...
    act = function(s)
        s.done = true
        return "Нет, показалось. Из узкого, как бойница, иллюминатора лишь вливается в коридор густая темнота. Мне хочется увидеть хоть что-то, кроме голых переборок и умирающего света. Я вспоминаю про иллюминатор в своей каюте."
    end
}
-- endregion
...
es.room {
    ...
    disp = "Озеро",
    dsc = [[Я сижу в лодке. Темнота позади подгоняет вперёд, дышит в спину холодом, но и передо мной на расстоянии в несколько шагов собирается мрак.]],
    onexit = function(s, t)
        if not have("orb") or not _("orb").active then
            p "Я оборачиваюсь, и тут же всякое желание идти куда-либо пропадает. Света нет, а я не хочу нырять с головой в эту безраздельную темноту."
            return false
        end
    end,
    enter = function(s)
...
es.room {
    ...
    obj = { "darkness" },
    way = {
        path { "Идти назад", "death_intro" },
        path { "Идти вперёд", "dream2" }
    }
}

es.obj {
    nam = "darkness",
    dsc = "Я напряжённо вглядываюсь в плотную поволоку темноты, в которой различается какое-то смутное пугающее {движение}, игра теней.",
...
es.obj {
    ...
    tak = [[Я поднимаю сферу, сжимаю её в ладони и тут же понимаю, что это обычный карманный фонарь. Светит он совсем слабо, но всё же придаёт мне уверенности -- возможно, с ним я всё же смогу пройти сквозь темноту.
    ^Правда, стоит мне лишь направить фонарь на собравшийся вокруг мрак, как он тут же гаснет.]],
    inv = function(s)
        if s.active then
            s.active = false
            return "Я зачем-то выключаю фонарь, как будто хочу сдаться темноте."
        else
            s.active = true
            return "Я включаю фонарь. Его луч прорезает собравшийся вокруг меня мрак, едва доставая до стен."
        end
    end
}
-- endregion
...
es.obj {
    ...
    dsc = "{Ожившая темнота} становится всё ближе.",
    act = "Может, всё же и правда стоит повернуть назад?",
    used = function(s, w)
        if w.nam == "orb" and w.active then
            if not s.done then
                s.done = true
                all.orb.active = false
                return "Я взмахиваю фонарём, и он тут же гаснет. В лицо мне бьёт темнота."
            else
...
es.obj {
    ...
    count = 0,
    exam = false,
    dsc = "{Темнота} вокруг наполнена страшной теневой жизнью, обретающей силу, когда тускнеет свет. Она не даст мне пройти, весь этот мир теперь принадлежит только ей. Я здесь -- чужеродный элемент, в моих руках -- последняя искорка света, и скоро я тоже стану темнотой.",
    act = function(s)
        if not s.exam then
            s.exam = true
            s.count = 0
