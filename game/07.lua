...
es.obj {
    ...
    act = function(s)
        if s.opened then
            ...
            s.opened = true
            return "Я нажимаю кнопку на стене, шторма иллюминатора опускается, и взгляд упирается в пустоту. Я ничего не вижу -- ни планеты, ни звёзд, лишь муаровую темноту космического пространства."
        end
    end
}
...
es.obj {
    nam = "talk",
    dsc = "{Марутян} оживлённо разговаривет с Симоновой, которая сосредоточенно морщится и кивает, глядя на него так, словно ей не терпится поскорее отправиться по своим делам. Впрочем, перебивать руководителя она решается.",
    act = function(s)
...
es.obj {
    nam = "mirror",
    dsc = "отливает чернотой, пока не коснёшься его поверхности, и лишь тогда, от тепла рук, в нём появляется сумрачное {отражение} -- лицо в обрамлении темноты, которое едва получается узнать.",
    act = "Можно подумать, здешние обитатели не любят отражения. А по вечерам прячутся друг от друга в многочисленных отсеках."
}
