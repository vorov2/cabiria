...
if(a[1]=="@metaparser" and here().commandProcessor) then
inp=inp:sub(#a[1]+2)
--Избавляемся от кавычек в начале и в конце текста.
inp=inp:sub(2,#inp-1)
here():commandProcessor(inp)
if here().terminal then return here().terminal:draw(),true else return iface_cmd(self,"look")
end
end
end
...
function initializeProcessor()
    return {
        ...
        draw = function(s)
            ...
            if not instead.tiny then config.fnt:text(prompt, config.prompt.fg):draw(s.hdevice, 0, 5)
			--else timer:stop()
			end
...
function initializeTerminal()
    return {
        ...
        draw = function(s)
if instead.tiny and s.out then return s.out
            elseif not instead.tiny and s.drawn then
                return s.drawn
            end
if not instead.tiny then
            ...
            for i,v in ipairs(s.lines) do
                if v == "`" then v = " " end
                if v == "-" and not instead.tiny then
                    sprite.new("theme/line.png"):
                        draw(s.drawn, 0, (i - 1) * config.fh + 6)
                elseif v == "~" then
v="Режим ограниченной функциональности"
                    if not instead.tiny then
...
--v=v:gsub("[\r\n]","")
if not v:startsWith("<boot") then s.out=s.out and s.out.."\n"..v or v
end
end
            end
end
            return instead.tiny and s.out or s.drawn
        end,
        clear = function(s)
            s.drawn = false
            s.newline = false
...
function es.terminal(tab)
    ...
    tab.drawOutput = function(s)
        ...
        return instead.tiny and p(output) or output
    end
    ...
    tab.timer = function(s)
        ...
        if instead.tiny then return s.out else return ret or ret2 end
    end
